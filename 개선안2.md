...
3. **✅ 관리자 대시보드 기본 기능 100% 완료**
   - 실시간 통계, 시스템 상태, 네비게이### 🎯 다음 단계 (우선순위별)

1. **🔴 긴급: 크래시 게임 수동 캐시아웃 서버 오류 해결**
   - 백엔드에서 500 Internal Server Error 발생 중
   - 세션 상태 관리 또는 DB 트랜잭션 로직 점검 필요

2. **🟢 브라우저 실제 테스트**
   - 수정된 코드로 로그인 → 게임 플레이 → 사용자 정보 표시 검증
   - useGlobalStore 오류 해결과 동기화 개선 효과 확인

3. **🟡 전체 시스템 안정성 검증**
   - 모든 주요 페이지에서 사용자 정보 정상 표시 확인
   - 실시간 상태 업데이트 동작 검증
   - admin/123456 계정으로 로그인 성공, /admin 페이지 정상 접근

2. **✅ 이벤트 CRUD 기능 95% 완료**
   - UI 인터페이스 완전 구현, 백엔드 API 연동 준비 완료

3. **✅ 크래시 게임 로직 100% 정상화**
   - 지수분포 배수 계산, 데이터베이스 테이블, API 엔드포인트 완료
...
#### 문제 1: API 경로 중복 (`/api/api/...`)
- **원인**: `useGlobalSync.ts`에서 API 경로에 `api/` 포함, `unifiedApi.ts`에서 자동 `/api/` 추가
- **해결**: 경로에서 `api/` 제거 (`auth/me`, `users/balance` 등)
- **결과**: API 호출 정상화
