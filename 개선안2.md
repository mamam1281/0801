...
#### 문제 3: 프론트엔드 API 연동 실패 (진행 중)
- **증상**: 브라우저에서 로그인 시 "❌ 에러 내용: 잘못된 정보 또는 서버 거부"
- **원인**: 환경변수에서 `NEXT_PUBLIC_API_ORIGIN=http://backend:8000`으로 설정
- **분석**: 브라우저는 컨테이너 이름으로 접근 불가, localhost:8000 필요
- **조치**: .env.development에서 `NEXT_PUBLIC_API_ORIGIN=http://localhost:8000`으로 수정
- **상태**: 프론트엔드 컨테이너 재시작 완료, 테스트 대기 중

#### 문제 4: 프론트엔드 로그인 여전히 실패 (현재 진행 중)
- **증상**: 
  - 브라우저 콘솔: `[API 요청] POST /api/auth/login` → `[API 실패] POST /api/auth/login`
  - 에러 메시지: "잘못된 정보 또는 서버 거부"
  - 백엔드 로그에서 로그인 요청이 도달하지 않음
- **추가 진단**:
  - 프론트엔드 컨테이너 환경변수 확인 필요: `NEXT_PUBLIC_API_ORIGIN` 값 재확인
  - 브라우저 네트워크 탭에서 실제 요청 URL 확인 필요
  - CORS 설정 확인: 백엔드에서 `localhost:3000` 허용 여부
- **현재 상태**: 환경변수 수정 후 컨테이너 재시작했으나 문제 지속
- **다음 조치**: 
  1. 프론트엔드 컨테이너 환경변수 실제 적용 확인
  2. 브라우저에서 실제 API 호출 URL 검증  
  3. 백엔드 CORS 설정 점검
