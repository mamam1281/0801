## [2025-09-11 17:20] 🎯 게임 통계 누적 버그 완전 해결 - mergeGameStats 전면 제거

### 변경 요약
- **핵심 문제 해결**: 모든 게임 컴포넌트에서 `mergeGameStats` 호출 완전 제거하여 통계 누적 버그 근원 차단
- **영향 받은 컴포넌트**: NeonCrashGame.tsx(2곳), GachaSystem.tsx(2곳), NeonSlotGame.tsx(2곳), RockPaperScissorsGame.tsx(1곳)
- **서버 권위 패턴 적용**: 클라이언트 측 누적 대신 `syncAfterGame()` 서버 권위 동기화만 사용
- **Import 정리**: 모든 파일에서 mergeGameStats import 제거 및 중복 import 해결

### 기술적 세부사항
**1. NeonCrashGame.tsx 수정**:
