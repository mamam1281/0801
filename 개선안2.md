...
- 검증
  - 에디터 기준 타입 오류 0(두 파일 모두). 컨테이너 Playwright 재실행 결과: 37 total → 33 passed / 4 skipped / 0 failed. unifiedApi(429/500) PASS, Action History pagination PASS, ws_profile_update_smoke PASS(지연 경고 로그만 출력).
- 다음 단계
...
- 테스트/E2E(Playwright)
  ...
  - [ ] 회원가입→로그인→balance 확인 스펙 추가/그린화
    - 진행: signup_balance_smoke.spec.ts 추가(현재 1건 실패 → 셀렉터 완화 및 gold-quick 폴백 반영, 재실행 예정)
  - [ ] Realtime 심화 스펙: dev 전용 emit API로 profile_update/reward_granted/purchase_update 브로드캐스트 → UI 반영/중복 억제 검증(스킵 가드 포함)
