...
### 2025-08-31 – Playwright BASE_URL 실서버 확인 및 Deep 플래그 운용 확정

- 변경 요약
  - Playwright 러너가 참조하는 BASE_URL/API_BASE_URL을 확인: BASE_URL=http://frontend:3000, API_BASE_URL=http://backend:8000. 메인 compose에서 frontend는 Dockerfile.dev + start-dev.sh로 Next.js dev 서버를 직접 기동하며 /healthz 헬스체크 수행. 스텁/정적 서버 경로는 존재하지 않음을 코드/이미지 구성으로 검증.
  - 심화 스펙 전용 오버라이드 `docker-compose.playwright.deep.yml`로 E2E_REALTIME_DEEP=1 설정 시에만 deep 스펙 실행. 기본 러너 compose는 원상 유지(그린 베이스라인 보장).
- 검증
  - 컨테이너 Playwright: 42 total → 34 passed / 8 skipped / 0 failed (exit code 0). ws_profile_update_smoke는 경고 로그만 있고 PASS.
- 다음 단계
  1) dev emit 라우터 인증 프로브 추가(/api/test/realtime/emit/stats_update): 신규 회원가입 토큰으로 2xx 확인 → 매핑 스모크 언스킵.
  2) /api/games/stats/me 200 보장 및 UI 합계 파리티 스펙 추가.
  3) Game Dashboard 네비게이션 스모크 재활성 전 화면 안정화(요소 가시성/로드 대기 개선).

### 2025-08-31 – Game/Home 대시보드 화면 증발 원인 분석 및 핫픽스

- 변경 요약
  - 원인: 화면 키 리팩토링(`'home'→'home-dashboard'`, `'games'→'game-dashboard'`) 이후 일부 경로에서 구 키가 전달되어 `App.tsx`의 조건 분기가 매칭되지 않아 공백 렌더 발생.
  - 조치: `frontend/hooks/useAppNavigation.tsx`에 키 정규화(normalize) 추가. `navigate()`와 하단 네비 핸들러가 구 키를 자동으로 신 키로 변환하도록 수정.
- 검증
  - 로컬에서 `localStorage.E2E_FORCE_SCREEN = 'games'|'home'` 설정 시에도 각 화면 정상 진입 확인. 하단 탭 클릭 → `[data-testid="game-dashboard"]` 가시성 PASS.
- 다음 단계
  - 코드베이스 내 구 키 사용처 잔여 탐지/정리(로그 추가로 추적). 네비게이션 스펙 안정화 후 언스킵.
...
### 2025-08-31 – E2E 재실행 및 심화 플래그 오버라이드 추가

- 실행 결과(컨테이너): 42 total → 34 passed / 8 skipped / 0 failed. dev emit 의존 스펙(realtime_event_mapping, purchase_dedupe, deep)은 가드에 의해 스킵 유지.
- 추가: `docker-compose.playwright.deep.yml`(플래그 오버라이드) 도입. 필요 시 아래처럼 덮어써 심화 스펙 실행 가능.
  - 병합 사용: `-f docker-compose.yml -f docker-compose.playwright.yml -f docker-compose.playwright.deep.yml`
- 메모: base compose는 원상 복원해 유효 상태 유지. dev emit 인증 가용성(2xx) 확인 후 매핑/딥 스펙 자동 해제 예정.
...
- [x] Playwright 러너 → Next.js dev 서버(실제 앱) 대상 확인(스텁 아님)
