...
- [x] [Blocking] 컨테이너 내부 /health 200 및 /docs 노출 1회 확인(중복 실행 불필요)
- [x] [Blocking] Alembic heads 단일성 확인만 수행(업그레이드 금지, heads -v 단일 여부만) — 현재 c6a1b5e2e2b1 단일 head
- [⚠️] [Blocking] 프로덕션 시크릿 교체 여부 확인(.env.production의 JWT_SECRET_KEY/DB/Redis/Kafka 키 값 교체 필요)

- [ ] [Defer OK] Alembic upgrade head 실제 실행(운영은 배포 창구 외 유지보수로 수행)
- [ ] [Defer OK] 백엔드 pytest 전체 스위트 재검(현재 QUICK_SMOKE=1 스모크 통과)
- [ ] [Defer OK] /api/games/stats/me 안정화 후 E2E_REQUIRE_STATS_PARITY=1 승격
- [ ] [Defer OK] Pydantic v2 경고 정리(기능 영향 없음)
- [ ] [Defer OK] OpenAPI 재수출/드리프트 가드(스키마 변경 시만)

### [Blocking] 프로덕션 시크릿 교체 진행상황(현재: 교체 필요)

- 점검 결과(.env.production):
  - DATABASE_URL=…secure_password_here…
  - POSTGRES_PASSWORD=secure_password_here
  - REDIS_PASSWORD=change_this_for_prod
  - JWT_SECRET_KEY=super_secure_production_key_here
  - JWT_SECRET=super_secure_production_key_here
  - FRONTEND_URL/BACKEND_URL=your-domain.com 템플릿
  - KAFKA_ENABLED=1(운영 사용 전 부트스트랩 서버/보안 재확인 필요)

- 교체 가이드:
  - JWT_SECRET_KEY/JWT_SECRET: 32자 이상 난수(추천: 48~64자), 서로 다른 값 권장.
  - POSTGRES_PASSWORD/REDIS_PASSWORD: 20자 이상 난수, 특수문자 포함.
  - DATABASE_URL: 운영 호스트/DB/비밀번호 정확히 반영.
  - 도메인: FRONTEND_URL/BACKEND_URL 운영 도메인으로 교체.
  - Kafka 미사용 시 KAFKA_ENABLED=0, 사용 시 신뢰 가능한 bootstrap과 자격/SSL 설정 확인.

- 검증 절차(교체 후):
  1) .env.production 비밀값 변경 → Git 커밋 금지(비밀 채널로 배포 노드에만 반영)
  2) docker compose --env-file .env.production config 검사(옵션)
  3) 백엔드 컨테이너에서 /health 200 및 /docs 200 재확인
  4) 간단 스모크(pytest 소범위) 0 fail 확인
...
### Blocking(즉시 확인 필요)
- [x] Backend 컨테이너 내부 `/health` 200 및 `/docs` 노출 확인(1회)
- [x] Alembic heads 단일성 확인만 수행(`alembic heads -v`) — 현재 c6a1b5e2e2b1 단일 head(업그레이드는 금지 유지)
- [⚠️] `.env.production` 시크릿 교체 확인: JWT_SECRET_KEY, DATABASE_URL/POSTGRES_*, REDIS_PASSWORD, Kafka(KAFKA_*) 키(템플릿 값 잔존)

### 배포 전 준비(가능하면 이번 라운드 수행)
- [x] 데이터베이스 사전 백업(덤프 저장) — postgres 컨테이너: backup_20250831_111616.dump 생성
...
- 즉시 체크리스트(운영 전/개발 공통)
  - [ ] FRONTEND: `.env.development` 또는 compose에 `NEXT_PUBLIC_API_ORIGIN` 설정 확인
  - (적용) `cc-webapp/frontend/.env.development`에 `NEXT_PUBLIC_API_ORIGIN=http://localhost:8000` 추가
  - 컨테이너 실행 중이라면 프론트만 재시작 필요: `docker compose restart frontend`
  - [ ] FRONTEND: `@/store/globalStore` export 목록과 `ShopScreen.tsx`/`GachaSystem.tsx` import 일치 확인
  - [ ] BACKEND: `/health` 200 및 컨테이너 Healthy
...
3) 성공 기준(체크)
- [x] `alembic heads` 단일 head 유지(현재 기준 head: c6a1b5e2e2b1)
- [ ] `alembic upgrade head` 성공(Exit 0)
- [x] 최소 스모크 0 fail: `test_smoke_health.py`, `test_ci_alembic_guard.py`, `test_schema_drift_guard.py`
...
아래 항목은 상단 “배포 전 단일 체크리스트(최신)”를 세부화한 참고용입니다. 중복 체크 방지를 위해 상단 체크리스트만 상태를 표기합니다.
