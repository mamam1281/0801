## 변경 요약 (2025-09-01)
### (추가) 2025-09-01 23:05 KST — 레거시 잔액 동기화 훅 제거(호출처 전환)
- 변경 요약: 프론트에서 useBalanceSync 의존을 제거하고 useGlobalSync로 일원화. 변경 파일: `ShopScreen.tsx`(구매 후 reconcileBalance→syncBalance), `components/games/NeonSlotGame.tsx`(useBalanceSync import/사용 제거), `components/games/GachaSystem.tsx`(fallback reconcileBalance→syncBalance). 전역 체크리스트(`전역동기화_솔루션.md`) 상태 업데이트.
- 검증 결과: 타입 기준 컴파일러 오류 원인이던 `reconcileBalance` 미정의 문제 해결 예상. 상점/게임 경로는 서버 응답 balance 우선 반영 후 `syncAfterGame`→실패 시 `syncBalance` 순으로 정합 유지. 런타임/테스트는 다음 단계에서 컨테이너 Playwright로 확인 예정.
- 다음 단계: (1) 컨테이너 Playwright 1회 실행으로 회귀 점검 (2) 남은 `useDashboard` 호출처(HomeDashboard) 대체 (3) useBalanceSync 파일/exports 제거 PR 분리 제출.
