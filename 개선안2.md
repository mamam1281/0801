## 2025-09-02 – PowerShell 헬스체크 명령어 구문 오류 해결

### 변경 요약
- **문제**: 기존 PowerShell 명령어에서 구문 오류 발생
  ```powershell
  ...
  ```
  오류: `'=' 용어가 cmdlet, 함수, 스크립트 파일 또는 실행할 수 있는 프로그램 이름으로 인식되지 않습니다.`

- **해결책**: 3가지 헬스체크 도구 제공
  1. `health-check.ps1` - 완전한 기능의 PowerShell 헬스체크 도구 (상세 분석, 문제 해결 가이드 포함)
  2. `health-check.sh` - Bash 버전 (Linux/macOS 호환)
  3. `quick-health.ps1` - 간단한 원라이너 대체 도구

- **환경 설정 문제 해결**:
  - `.env.development` UTF-16 인코딩 → UTF-8 변환
  - Docker 네트워크 `ccnet` 생성
  - Zookeeper/Kafka 이미지를 Confluent 버전으로 업데이트 (bitnami 이미지 접근 불가 해결)

### 검증 결과
- 모든 Docker 서비스 정상 시작 및 헬스체크 통과:
  - Backend: http://localhost:8000/health → 200 OK
  - Frontend: http://localhost:3000/healthz → 200 OK
- 새로운 헬스체크 스크립트들 정상 동작 확인
- Docker Compose 서비스 상태: backend, frontend, postgres, redis, kafka, zookeeper 모두 healthy

### 다음 단계
- [ ] PowerShell 사용자들에게 새로운 헬스체크 도구 사용법 안내
- [ ] 기존 복잡한 명령어를 `.\quick-health.ps1`로 대체
- [ ] 개발 문서에 새로운 헬스체크 가이드 링크 추가
- [ ] CI/CD 파이프라인에서 `./health-check.sh` 사용 검토
