...
### 2025-08-14 – 상점 구매 UX/히스토리/E2E 보강

- 변경 요약
	- 프론트: ShopScreen에 “구매 히스토리” 진입 링크 추가, `app/shop/history/page.tsx`(클라이언트 컴포넌트) 신설 및 `/api/shop/transactions?limit=50` 연동(`utils/shopApi.ts::apiGet`) — 로딩/오류/빈 목록 처리, 글래스메탈 톤 유지.
	- 구매 플로우: 커스텀 오버레이 모달 복원, 상단 얇은 네온 그라데이션 라인 미세 효과 추가(레이아웃 불변). 통화(currency)에 한해 수량 스텝퍼(±/입력) 활성화, 서버 payload에 `quantity`와 `amount=단가×수량` 반영. 로컬 상태(잔액/획득량/인벤토리) 동기화.
	- 접근성(a11y): 수량 컨트롤(− 버튼/숫자 입력/+ 버튼)에 `aria-label` 추가(시각적 변경 없음).
	- E2E: `frontend/tests/e2e/shop-buy.spec.ts`에 성공 플로우, 실패+재시도, 수량 증가 스모크 케이스 추가. 실패 경로 안정화를 위해 비프로덕션 환경에서 URL 쿼리 `?e2eFail=once` 사용 시 1회 실패 모킹 로직을 `utils/actions.ts`에 도입.

- 검증 결과
	- 프론트 타입체크 PASS(`npm run -s type-check`). OpenAPI 변경 없음 → 재수출 불필요.
	- a11y 라벨 추가 이후 빌드/타입체크 영향 없음.
	- UI 회귀 없음(글래스메탈/네온 톤 유지, 배치 불변). 히스토리 페이지는 토큰 보유 전제로 클라이언트 fetch 동작.

- 다음 단계
	- 히스토리 API 스키마 확정 시 칼럼/정렬 보강(예: product_name, created_at, amount, quantity, status).
	- Playwright E2E를 CI에 연결하여 회귀 방지(실패 경로는 모킹/플래그로 결정론 확보 검토).
	- 비통화 아이템 수량 정책 확정 후 UI 확장 여부 결정. 접근성(aria-label, 포커스 순서) 및 잔액 부족 툴팁 가이드 보강.


### 2025-08-14 – 알림 설정 E2E 및 CI 통합

- 변경 요약
	- 프론트: 알림 설정 페이지 E2E 추가(토글 지속성, 타입 음소거, SSE 선호, 푸시 권한 요청 UI 스텁). Playwright 설정에 dev 서버 자동 기동(webServer)과 TS 타입체크 게이트 추가.
	- 백엔드: 런타임 스모크 테스트 추가(test_smoke_notifications_runtime.py)로 REST push/list/read 및 WS 백필 경로 검증. CI에서 Alembic 다운/업 가드와 함께 실행.
	- CI: `.github/workflows/ci.yml`에 frontend-e2e 잡 신설(Next dev, Playwright 설치, 타입체크, 테스트 실행), backend-tests 잡에 알림 런타임 스모크 포함.

- 검증 결과
	- 타입체크(프론트) PASS, Playwright 알림 설정 E2E PASS.
	- Alembic upgrade head PASS, 백엔드 런타임 스모크 PASS(health/docs/WS/SSE/REST 연동 경로).
	- OpenAPI 변경 없음(엔드포인트 불변) → 재수출 불필요.

- 다음 단계
	- SSE 이벤트 스트림 직접 파싱 E2E는 선택 과제(필요 시 추가).
	- 문서/체크리스트 최신 상태 유지(본 문서 및 `20250841-001.md` 업데이트 지속).
	- CI 플레이크 모니터링 및 a11y/SEO 기준치 조정 검토.

### 2025-08-14 – 결제 모달 스코프 조정(내부 포인트/관리자 지급)

- 배경
	- 본 서비스는 외부 PG 결제가 아닌 내부 포인트(젬/토큰) 소비 및 관리자 지급(어드민) 정책을 채택.
	- 따라서 결제수단 선택/보안 인증서/영수증 발행은 스코프 아웃(N/A).

- 변경 사항
	- 프론트 결제 모달 → "포인트 차감 확인 모달"로 단순화(항목: 상품명, 차감 포인트, 남은 잔액 예상, 확인/취소).
	- 보안 표시는 서버 응답의 transaction_id 및 서명 검증 결과로 대체(백엔드 로깅/감사에 집중).
	- 완료 플로우: 완료 토스트 + 구매 내역 반영(히스토리 페이지)로 대체, 영수증 다운로드/이메일 발송은 미구현.

- 검증 기준(대체)
	- 차감 전 확인 모달 노출, 진행 상태 표시(로딩 스피너), 오류 메시지/재시도 버튼 제공.
	- 성공 시 잔액·인벤토리 갱신, 완료 토스트와 상세 링크(히스토리) 제공.

- 다음 단계
	- Shop UI에 확인 모달 추가 및 `/api/shop/buy` 연동.
	- Playwright E2E 2케이스: 성공/오류·재시도.
	- 어드민 포인트 지급 UI는 별도 섹션(관리자 기능)에서 진행.
