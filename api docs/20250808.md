...
### 2025-09-01 – 관리 스크립트 헬스체크 포트 폴백 추가 및 검증

- 변경 요약: `cc-manage.ps1`의 `Check-Health`가 오버라이드 존재 시 8001/3001만 단일 시도하던 문제를 개선. 현재 계산된 포트에서 실패 시 자동으로 (8000/3000) ↔ (8001/3001) 포트 쌍을 순차 폴백해 실제 노출 포트를 탐색/검증하도록 수정. `.env.local`의 `BACKEND_PORT`/`FRONTEND_PORT` 지정값은 최우선 반영.
- 검증 결과: 로컬 환경에서 override 파일 존재로 8001/3001 시도가 실패했으나 폴백으로 8000/3000에서 `API /health => healthy`, `Web / => 200` 확인. `./cc-manage.ps1 status` 기준 backend/frontend 모두 healthy, 포트 매핑 8000/3000. 컨테이너 내부 `pytest -q app/tests/test_smoke_health.py` 2 passed. Alembic `heads -v` 단일 head(c6a1b5e2e2b1).
- 다음 단계: (1) `.env.local`에 포트를 명시해 개발자별 일관성 강화 권고 (2) 프론트 전용 `/healthz` 경량 엔드포인트 도입 검토(healthcheck 속도/안정성 향상) (3) cc-manage의 `health` 출력에 현재 성공 포트 기입(완료) 및 실패 시 힌트 메시지 유지.
