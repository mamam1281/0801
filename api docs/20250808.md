...
### 2025-08-26 – 보상 즉시 반영(HTTP/WS 일관화) 및 중복 제거

- 변경 요약
	- Event/Mission/스트릭 HTTP 성공 응답 시 즉시 전역 스토어에 보상을 반영하도록 연결: `applyReward(dispatch, { reward_data: response.rewards })` 사용. 진행도(progress)와 claimed 상태는 서버 재호출로만 갱신하여 권위 유지.
	- `store/globalStore.ts` 내 중복 선언된 `applyReward`를 제거하여 단일 구현 유지(WS/HTTP 공용).

- 검증 결과
	- 프론트 타입체크 통과. 수동 스모크: 스트릭 보상/이벤트·미션 클레임 후 골드 뱃지 즉시 가산 → 이어서 `/api/users/balance` 재동기화로 최종 정합.
	- OpenAPI/Alembic 변경 없음. `/health` 200 및 `/docs` 정상.

- 다음 단계
	- Playwright 스모크 업데이트: “HTTP 보상 성공 → UI 즉시 반영 → balance 재동기화 후 일치” 케이스 추가.
	- Realtime `reward_granted` 수신 시 중복 토스트 억제 로직 정비 및 최근 보상 로그 UI 연동.
	- Crash/가챠 등 나머지 보상 경로도 동일 패턴 재점검.

### 2025-08-26 – 상점 카탈로그 서버화 & purchase_update 토스트 연결

### 2025-08-27 – 토큰 마이그레이션/인증 헤더 안정화 및 WS→UI 스모크 그린화 대응

- 변경 요약
	- App 부트스트랩 시 `ensureTokenBundleMigrated()`를 즉시 실행하여 레거시 `cc_access_token` → `cc_auth_tokens` 번들 자동 생성 보장.
	- 전역 fetch 패치: `/api/*` 요청에서 Authorization 헤더가 없으면 cc_auth_tokens 또는 cc_at 쿠키에서 Bearer 토큰 자동 주입.
	- RealtimeSyncContext의 `reward_granted` 수신 시 `applyReward`로 전역 스토어 잔액 즉시 반영(기존 recent_rewards 유지) 재확인.

- 검증 결과
	- Playwright: auth_migration.spec.ts 및 ws_profile_update_smoke.spec.ts의 401/빈 번들 이슈 재현 케이스 해소 예상. 컨테이너 실행으로 재검증 진행 필요.
	- 백엔드 스키마/OpenAPI 변경 없음.

- 다음 단계
	- 컨테이너 내 E2E 재실행 및 스냅샷 갱신. 실패 시 쿠키 경로/도메인 보강 또는 Next middleware로 SSR fetch 주입 검토.
	- fetch 패치가 중복 적용되지 않도록 가드 유지(플러그인화 가능성 검토).

- 변경 요약(추가)
	- 백엔드 FastAPI 메인앱에 `/api/health` 별칭 엔드포인트 추가(기존 `/health`와 동일 응답). 컨테이너/테스트 스택에서 `/api/health`를 폴링하는 스크립트와의 호환성을 확보해 Playwright 글로벌 셋업 초기 5xx를 제거.

- 검증(추가)
	- `GET /health` 200, `GET /api/health` 200 동등 확인.
	- docker-compose 헬스체크는 `/health`를 계속 사용, Playwright 글로벌 셋업은 `/health` 우선 + 루트 폴백 유지.

- 다음 단계(추가)
	- 컨테이너 E2E 러너 재실행으로 전체 스위트 그린 검증.
	- 문서 전반에서 권장 헬스 경로를 `/health`로 통일, 호환 경로(`/api/health`)는 유지.
