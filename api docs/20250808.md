...
### 2025-08-27 – 전역 스토어 확장 및 통일 API 하드닝(잔액 권위/토스트 연동)

- 변경 요약
	- `frontend/store/globalStore.ts`를 확장하여 상태에 `balances, streak, events, notifications, lastError`를 추가하고, 액션/헬퍼 `hydrateFromServer, reconcileBalance, applyReward`를 도입했습니다. 기존 `mergeGameStats, applyPurchase`와 함께 서버 권위 흐름을 일원화했습니다.
	- `frontend/lib/unifiedApi.ts`는 비-GET 요청 시 `X-Idempotency-Key` 자동 주입 옵션을 노출하고, 재시도 가능한 상태코드에 `409`를 포함하도록 강화했습니다. 상대경로 사용 시 '/api' 접두 자동 부여 정책을 재확인했습니다.
	- 실시간 이벤트(`profile_update/reward_granted/purchase_update`)는 윈도우 이벤트(`app:notification`)로 토스트에 전달되어 Provider 결합도를 낮추고, 1.5초 중복 억제를 유지합니다.

- 검증
	- 타입/린트: 프론트 타입 오류 없음(로컬 분석 기준). 기존 컴포넌트의 selector 사용 흐름과 호환됩니다.
	- 런타임 가정: 컨테이너 내부 Playwright 및 수동 스모크에서 `/api/auth/me` 초기 하이드레이션과 `/api/users/balance` 재동기화로 잔액 일관성이 유지되어야 합니다(테스트는 컨테이너 내 실행 원칙).
	- 백엔드/OpenAPI/Alembic: 스키마 변경 없음. /health 200, /docs 기존 상태 유지 예상.

- 다음 단계
	1) 컨테이너 내부에서 E2E(WS→UI 스모크 포함) 재실행하여 전역 스토어 확장 영향 확인.
	2) `ProfileScreen`/게임/상점 경로에서 새 액션(`applyReward/reconcileBalance`) 사용 여부 재점검 및 간단 스모크 추가.
	3) 필요 시 OpenAPI 재수출 및 본 문서에 결과 업데이트.

### 2025-08-27 – 게임 UX 하드닝(오류 배너+재시도, 서버 권위 캐시아웃)

- 변경 요약
	- RPS(가위바위보), Gacha, Crash 컴포넌트에 네트워크 실패 시 사용자에게 명확히 안내하는 오류 배너와 “재시도” 버튼을 추가했습니다. 재시도는 마지막 액션(choice, pull type 등)을 보존해 즉시 수행합니다.
	- Crash: bet→애니메이션→cashout 흐름이 서버 권위를 따르며, 캐시아웃 요청 실패 시에도 배너를 통해 재시도 제공. 성공·실패와 무관하게 withReconcile로 users/balance를 최종 일치시킵니다.
	- Gacha: 서버 응답 시 applyPurchase/mergeGameStats/mergeProfile로 반영, 실패 시엔 시각효과만 폴백하며 로컬 잔액 변경은 없습니다.

- 검증
	- 타입/린트: 수정 파일 내 타입 오류 없음(IDE 기준).
	- 런타임: 네트워크 차단/지연 상황에서 배너 표시 및 재시도 동작을 수동 스모크. 성공 시 잔액은 `/api/users/balance`와 일치(WS 미수신 시에도 withReconcile 폴백으로 일치).

- 다음 단계
	1) Slot에도 동일한 오류 배너+재시도 UI 적용하여 4개 게임 일관성 완성.
	2) Playwright에 “게임 실패 시 배너 노출 및 재시도 성공 후 잔액 일치” 스펙 추가.
	3) 필요 시 OpenAPI 재수출 점검(프론트만 변경, 백엔드 스키마 무변경).

### 2025-08-27 – Providers 구성 정리(Store→Hydrate→Realtime→Toast 일관화)

- 변경 요약
	- `frontend/app/providers.tsx`에서 `EnsureHydrated`/`RealtimeSyncProvider` 중복 래핑을 제거하고, `ToastProvider`와 UI 전용 `FeedbackProvider`만 유지했습니다. 전역 스토어 및 실시간/하이드레이션 구성은 `App.tsx`에서 `GlobalStoreProvider → EnsureHydrated → RealtimeSyncProvider` 순으로 적용됩니다. 이로써 문서 명세(Providers 순서)와 실제 코드 구성이 일치하며, 초기 토큰 마이그레이션/하이드레이션 레이스로 인한 WS 초기 연결 타이밍 이슈 가능성을 줄였습니다.

- 검증
	- 빌드/타입: 프론트 타입 오류 없음(IDE 기준), SSR 레이아웃은 `ToastProvider`만 상위에 존재해 Store 훅 호출이 SSR 중 발생하지 않음을 보장합니다.
	- 런타임: `App.tsx`에서 Store→Hydrate→Realtime→Toast 순서가 유지됨을 수동 점검. WS 연동은 기존과 동일하게 동작하며 토스트는 최하단(provider 체인 마지막)에서 안전하게 표시됩니다.

- 다음 단계
	1) Playwright WS 스모크에서 초기 하이드레이션 직후 `fetch('/api/auth/me')` 재시도(200~300ms) 보강으로 간헐적 401 방지 확인.
	2) `TokenBalanceWidget` 상세 모달(최근 변경 이력) 구현 후 전역 스토어의 `recent_rewards/recent_purchases`와 연동.
	3) 필요 시 OpenAPI 재수출 점검 및 본 문서 추가 갱신.
