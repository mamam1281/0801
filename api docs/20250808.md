## 2025-08-13 업데이트 (Shop & Economy)
\n+변경 요약 (Alembic 정리 & 한정 패키지 테이블)
- Alembic 단일 head 유지 재정비: 중복 목적 리비전 `20250813_add_shop_tables` 제거, 체인은 `20250813_user_actions_ix_type_created → 20250813_create_shop_tables → 20250813_add_limited_packages_and_promos`로 단일화
- 신규 테이블 추가: `shop_limited_packages`, `shop_promo_codes` (idempotent 생성, 인덱스/유니크 포함)
- 모델 정합성: `app.models.__init__`에서 ShopLimitedPackage/ShopPromoCode를 `shop_models`로부터 내보내도록 통일

검증 결과
- pytest 스모크(인증/블랙리스트/리프레시): PASS (12 + 3 케이스), Pydantic v2 경고는 잔존(차후 ConfigDict 이행 예정)
- Alembic 체인 점검: heads 단일 유지, down_revision 연결 정상
- 한정 패키지 E2E 테스트는 관리자 가드로 현재 skip, 마이그레이션 적용 후 시나리오 확장 예정

다음 단계
- 컨테이너 내부에서 `alembic upgrade head` 실행 후 `/docs` 확인 및 OpenAPI 재수출 필요 시 `python -m app.export_openapi`
- 한정 패키지/프로모 코드 테스트 보강: 재고/사용자별 제한/긴급 비활성화/프로모 유효기간 및 사용량
- Pydantic v2 마이그레이션: class-based Config → ConfigDict 단계적 적용

변경 요약
- 경제 구조 단일 화폐(cyber_token_balance) 유지 확정
- 상점 구매 API 추가: POST /api/shop/buy (kind=gems|item)
	- gems: 코인 충전(PURCHASE_GEMS 로그)
	- item: 코인 차감 + 구매 기록(BUY_PACKAGE 로그)
- Alembic: user_actions(action_type, created_at) 인덱스 추가 리비전 작성
- FE 헬퍼 추가: frontend/utils/actions.ts (postAction, postActionsBatch, buyProduct)

검증 결과
- pytest: app/tests/test_shop_buy_api.py → PASS (충전/과금 실패/정상 구매 시나리오)
- 기존 인증 스모크 테스트 → PASS, heads 단일 체인 유지 (신규 리비전: 20250813_user_actions_ix_type_created)

다음 단계
- 상품 카탈로그/가격/할인 테이블 도입 및 검증 로직 강화
- 영수증(트랜잭션 로그) 발행/저장 및 조회 API 추가
- 결제 게이트웨이 연동(모의/샌드박스) 및 실패/환불 플로우 정의

## 2025-08-13 업데이트

변경 요약
- 실시간 알림(WS/SSE) 기능 완성: 토픽 필터, 다중 클라이언트, 재연결 백필, 정상 해제 처리 보강
- 사용자 행동 로깅 API 추가: POST /api/actions, POST /api/actions/batch (PII 마스킹, DB 저장, 선택적 Kafka 발행)
- 관련 통합 테스트 추가 및 정비(리얼타임/액션)

검증 결과
- pytest: app/tests/test_realtime_notifications.py, app/tests/test_actions_api.py → PASS
- /docs 노출 정상, WS/SSE 스모크 OK, Last-Event-ID 재개 동작 확인
- Alembic heads 단일 유지(신규 마이그레이션 불필요)

다음 단계
- user_actions 인덱스: (user_id, created_at), (action_type, created_at) 추가 검토 및 적용
- 로그 보관/압축 정책 수립과 주기 아카이빙 파이프라인 설계
- FE 주요 플로우에 /api/actions 연동 및 계측 대시보드 확장

### 2025-08-12 – Post-merge validation (upstream-preferred)

- 변경 요약: upstream/main 병합 이후 OpenAPI 재수출 및 핵심 인증 스모크 테스트 재검증. 게임 가챠 경로는 통일된 `/api/games/gacha/pull`만 유지.
- 검증: `python -m app.export_openapi` 성공, auth 스모크 12개 PASS, Alembic head 가드 PASS, `/health` OK.
- 다음 단계: FE 타입 재생성(선택, openapi-typescript), 레거시 `app/routers/gacha.py` 완전 제거 여부 팀 합의 후 진행, 전체 테스트 스윗 주기적 실행.
...
### 2025-08-13 – API path/tag de-dup cleanup

- 변경 요약:
	- Actions: 중복 경로 `/api/actions/actions` 제거 → 표준 `POST /api/actions`로 통일.
	- Rewards: 태그 대소문자 혼용(["Rewards"] vs ["rewards"]) 정리 → ["Rewards"] 단일화.
	- Games: `main.py`에서 태그 오버라이드 제거로 "Games"/"Game Collection" 중복 섹션 해소 (라우터 내부 태그만 사용).
- 검증 결과:
	- `pytest` 인증 스모크 12개 PASS (변경 영향 없음).
	- `python -m app.export_openapi` 성공, backend/current_openapi.json 갱신.
	- Swagger `/docs`에서 Rewards/Games 섹션 중복 미발생 확인.
- 다음 단계:
	- FE에서 사용 중인 `/api/actions/actions` 호출이 있다면 `/api/actions`로 경로 갱신.
	- 필요 시 openapi-typescript 재생성 및 타입 업데이트.
	- 라우터 추가 시 태그/프리픽스 일관성 점검 체크리스트에 추가.

추가 (2025-08-13)
- 신규 스크립트: `scripts/test-ws.ps1` — WS 연결/구독/해지/LastEventId 백필 테스트용.
- 백엔드 README에 WS/SSE 파워셸 예제 추가.
- 레거시 `app/websockets.py` 및 `app/websockets/__init__.py`는 `app.realtime.manager`로 셈링크 수준 리익스포트(Deprecated 표기). 신규 기능은 `app/realtime.py`만 사용.
...
검증 스냅샷
- OpenAPI 재수출 성공: `python -m app.export_openapi` → `current_openapi.json` 갱신
- Auth 플로우 테스트 통과: refresh, signup/login/me, lockout, `/api/auth/me` alias 동작 확인
