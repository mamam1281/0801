## 2025-09-21 – Alembic 단일 head 고정 및 안전 리프레시 SOP (PREPEND)

### 변경 요약
- 다중 head를 머지 리비전으로 통합하고, DB에는 `upgrade`가 아닌 `stamp head`를 먼저 적용하여 초기 스키마 재적용으로 인한 `DuplicateTable` 리스크 제거.
- PowerShell→컨테이너 bash 인용 규칙을 정리하여 psql 커맨드 실패를 방지.
- 테스트 가드 추가: `test_alembic_policy.py`에서 heads가 1개인지 상시 검증.

### 검증
- `alembic heads` → 단일 head 1개만 노출
- `alembic current` → DB 버전 테이블에 최신 head 표시
- `pytest -q app/tests/test_alembic_policy.py` → PASS

### 절차
1) 상태 확인: `docker compose exec -T backend bash -lc 'alembic heads; echo ---; alembic current'`
2) 병합 필요 시: `docker compose exec -T backend bash -lc "alembic merge -m 'merge heads unify' --rev-id 20250921_merge_heads_unify <HEAD_A> <HEAD_B> [<HEAD_C> ...]"`
3) 스탬프: `docker compose exec -T backend bash -lc "alembic stamp head; alembic current"`
4) (옵션) 업그레이드 no-op 확인: `docker compose exec -T backend bash -lc 'alembic upgrade head || true'`

### PowerShell 안전 인용 예시
- `docker compose exec -T postgres bash -lc 'PGPASSWORD=cc_password psql -U cc_user -d cc_webapp -c "SELECT * FROM alembic_version;"'`

### 다음 단계
- 본 SOP를 지속 적용하고, 릴리스 전 `alembic heads`와 `pytest`를 게이트로 운영.

## 2025-09-20 – Events/Missions 500 핫픽스 및 회귀 검증

### 변경 요약
- 미션 진행 업데이트 라우터(`/api/events/missions/progress`)가 `mission_id` 기반 서비스 메서드(`MissionService.update_mission_progress_by_mission_id`)를 호출하도록 정합화하여 시그니처 불일치로 인한 500 차단.
- 이벤트/미션 직렬화 안정화: 서비스 레이어에서 `rewards`/`requirements`(NULL→`{}`) 및 `current_progress`(NULL→`0`) 안전 보정 추가로 Pydantic 검증 실패 방지.
- 기존 스타 임포트 제거로 심볼 충돌 리스크 축소(서비스 내부).
 - 타입 안전 리팩터: `event_service.py`의 이벤트/미션 경로에서 SQLAlchemy Column에 대한 직접 대입을 피하고 `getattr/setattr` + 안전 캐스팅(`_to_int/_to_bool/_to_dict`)을 도입하여 정적 분석 경고(할당 불가/ColumnElement bool 등) 제거.
 - `claim_event_rewards` 들여쓰기/스코프 오류 수정 및 `claimed_at` 기록 추가.

### 검증
- 컨테이너형 Playwright 스위트: 31 passed / 12 skipped / 6 failed (총 49). 이벤트/미션 목록 로드 중 500은 재현되지 않음. 실패 항목은 어드민 UI가드, 레거시 토큰 마이그레이션, 데일리 리워드 UX 등 비관련 영역에서 발생(회귀 영향 없음).
- 정적 분석(Pylance): 수정 전 보고되던 `Column[...]`에 직접 대입/비교 관련 오류가 이벤트/미션 서비스 파일에서 0건으로 감소. 워크스페이스 전역 스캔은 잔여 테스트 헬퍼(타입 힌트 None 허용)와 레거시 `services/mission_service.py`에서만 경고 확인됨(다음 라운드 정리 예정).
- OpenAPI 스키마 변경 없음(스키마 파일 미변경).

### 다음 단계
1) 백엔드 pytest 스모크(선택): 컨테이너 내부에서 `pytest -q app/tests -k events`로 주요 이벤트/미션 경로 확인.
2) 필요 시 `/api/events`·`/api/events/missions/daily` 수동 스모크(401→로그인→200) 추가 점검.
3) 레거시 `app/services/mission_service.py`는 현행 모델과 불일치하므로 사용처 점검 후 제거 또는 호환 래퍼로 통합(중복 금지 원칙 준수).
4) 테스트 헬퍼의 함수 시그니처에 `Optional` 반영 등 타입 힌트 정합화.
