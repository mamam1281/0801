### 2025-08-11 업데이트: 초대코드 및 경고 정리

- 초대코드: InviteCodeRepository 도입으로 '5858' 무제한, 그 외 단일 사용 로직 확립. Auth 라우터에 검증/소비 연동.
- 테스트: invite repo 3개 케이스 통과, 리그레션 번들(리프레시/사인업→로그인→me/헬스) 10 passed.
- 경고: httpx TestClient 및 passlib crypt 경고는 pytest 필터로 억제, pydantic config deprecation은 추후 점진 대응 예정.
- 운영: Admin 엔드포인트로 초대코드 목록/시드 지원 추가.
...
## 1.1 변경 요약 (2025-08-11)

- 로깅 미들웨어가 응답 헤더에 X-Request-ID를 추가하도록 개선(요청-응답 상관관계 추적 강화)
- 인증 서비스 단위 테스트 추가: 비밀번호 해시/검증, JWT 생성/검증, 만료 처리, 서명 알고리즘 확인
- OpenAPI 재검증: /api/auth/refresh, /api/users/vip-only 경로 포함 확인 유지
 - /api/auth/refresh 본문 토큰 허용 안정화: HTTPBearer(auto_error=False)로 조정하여 Authorization 헤더 없이도 정상 처리

검증 결과
- pytest -q app/tests 중 신설 유닛 테스트 통과(로컬/컨테이너 동일 환경에서 실행)
- /health 200, /docs 정상 노출, X-Request-ID 응답 헤더 존재 확인
 - 리프레시 플로우 검증: app/tests/test_auth_refresh_flow.py 2/2 통과 (Bearer/Body 모두)

다음 단계
- 경고 정리(Protected namespace, httpx 버전 고정) 및 커버리지 확장(서비스/리포지토리 레벨)
- JWT 로테이션/블랙리스트(강제 로그아웃) 설계 초안 작성 및 단계적 반영
- Alembic 단일 head 유지 확인 후 필요 시 merge 리비전 계획 수립

## 8.x 프론트엔드 로컬 개발 포트 바인딩/헬스체크 (2025-08-11)

증상
- 컨테이너는 기동되나 호스트에서 http://127.0.0.1:3001 접속이 간헐적으로 실패하거나 초기 단계에서 연결 종료.

원인
- Next.js dev 서버 바인딩(0.0.0.0) 및 초기 설치/컴파일 타이밍으로 인한 헬스체크 실패.

조치
- cc-webapp/frontend/package.json: `dev` 스크립트 → `next dev --hostname 0.0.0.0 --port 3000`.
- docker-compose.override.local.yml: 중복 command/HOSTNAME/PORT 제거, 포트 매핑만 유지(3001:3000), 헬스체크 보강(start_period=30s, retries=10).

검증
- 컨테이너 내부: `ss -ltnp | grep 3000` → 0.0.0.0:3000 LISTEN (next-server)
- 호스트: 브라우저로 http://127.0.0.1:3001 접속 OK. PowerShell Invoke-WebRequest는 keep-alive 처리 차이로 간헐 오류 메시지 출력될 수 있으나 실제 응답은 200 OK 확인됨.

### 8.y Kafka 로컬 라운드트립 검증 (2025-08-11)

환경
- docker-compose.override.local.yml에서 백엔드에 Kafka 활성화 환경변수 설정
	- KAFKA_ENABLED=1
	- KAFKA_BOOTSTRAP_SERVERS=kafka:9092
	- KAFKA_TOPICS=cc_test
	- KAFKA_CONSUMER_GROUP=cc_backend_group

검증 절차(호스트 PowerShell)
1) 백엔드 헬스/레디니스 확인
	 - GET http://127.0.0.1:8001/health → { status: healthy }
	 - GET http://127.0.0.1:8001/api/kafka/_debug/ready → { ready: true }
2) 메시지 생산(고유 마커 포함) 및 피크 확인
	 - POST http://127.0.0.1:8001/api/kafka/produce, body: { topic: "cc_test", payload: { marker: "e2e-<guid>" } }
	 - GET http://127.0.0.1:8001/api/kafka/debug/peek?topic=cc_test&max_messages=200&from_beginning=1
	 - 응답 내 marker 존재 확인 → 라운드트립 OK

샘플 원라이너
- PowerShell
	- 고유 마커로 produce → peek에서 포함 여부 확인, 성공 시 'peek contains marker: OK' 출력

비고
- 컨테이너 내부에는 wget이 없을 수 있으므로 호스트에서 Invoke-RestMethod 또는 curl.exe 사용을 권장
- JSON 전송 시 PowerShell 객체를 ConvertTo-Json 후 Content-Type: application/json으로 전송

### 8.z 테스트 경고 정리 계획 (후속 PR)

- Pydantic v2 경고: class-based config→ConfigDict로 전환, model_ 네임스페이스 충돌 필드 리네임 또는 protected_namespaces 조정
- httpx deprecation: TestClient 경로가 요구하는 버전에 맞춰 httpx 0.27.x로 핀 고정, 코드 내 app= 전달 경고 문구 해결

### 8.z.1 추가 단위 테스트 및 경고 저감(2025-08-11)

변경 요약
- AuthService 유닛 테스트 추가: 해시/검증, JWT 생성/검증, 만료 처리, alg/key sanity
- UserRepository 기본 CRUD 라운드트립 테스트 추가 (테이블 생성 포함)
- 요청 추적을 위한 X-Request-ID 응답 헤더 주입(미들웨어)

검증
- 컨테이너 내부에서 단위 테스트 실행: PASS (3개 파일)
- /health 응답 헤더에 x-request-id 존재 확인

다음 단계
- 경고 저감: pydantic protected_namespaces 설정, passlib crypt 경고 억제, httpx 고정 버전 재확인
- 리포지토리/서비스 추가 유닛 테스트 확장(통계/검색/세그먼트)

---

## 11. 토큰 로테이션/블랙리스트(강제 로그아웃) 설계 초안

목표
- 보안 사고 대응 및 세션 강제 무효화를 지원 (계정 탈취 의심, 권한 변경, 규정 위반)

데이터 모델
- token_blacklist (id, token|jti, user_id, reason, blacklisted_at, expires_at)
- refresh_tokens (기존 모델 활용: is_revoked, expires_at)

프로세스
1) 로그인/리프레시 발급 시 jti 포함(access/refresh 둘 다)
2) 로그아웃/강제무효화 시: 해당 jti 또는 토큰 원문을 token_blacklist에 삽입, refresh_tokens.is_revoked=true
3) 인증 미들웨어: JWT 검증 후 jti/토큰이 블랙리스트에 존재하는지 O(1) 캐시 조회(Redis) → 존재 시 401
4) 만료/청소: expires_at 지난 항목은 주기적으로 삭제 (백그라운드 작업)

API 변화(초안)
- POST /api/auth/logout: 현재 세션의 access/refresh를 모두 무효화(블랙리스트 삽입+refresh revoke)
- POST /api/admin/sessions/revoke (관리자): 특정 사용자/세션 jti 단위로 강제 무효화

점진 배포 계획
- Phase 1: 모델/리포지토리/서비스 레이어 구현, 블랙리스트 체크를 인증 경로에 read-only로 연결(미스/히트 로깅)
- Phase 2: /logout 구현 및 클라이언트 동작 정렬, 캐시(Redis) 온-더-사이드 도입
- Phase 3: 관리자 강제 무효화/감사 로그, 지표 대시보드 추가

주의 사항
- 블랙리스트 저장소 장애 시 fail-closed 정책(권장) 또는 임시 grace(운영 판단) 선택지 제공
- jti 충돌 방지(UUIDv4)와 clock skew 고려(exp 여유) 필요
- passlib/crypt 경고: Python 3.13 대비 bcrypt 백엔드만 사용 확인 및 crypt 의존 제거 검토
- 마커 경고: pytest.ini에 필요한 플러그인 등록 또는 해당 마커 제거/대체
- 목표: 경고 수 70% 이상 절감, CI 로그 소음 저감
...
- [ ] 전체 테스트 통과 (크래시 라이프사이클은 토큰 제공 시 통과)
...
### OpenAPI 재생성/검증 스니펫 (컨테이너 내부 실행)

- 재생성
	- docker exec cc_backend_local python -m app.export_openapi
- 핵심 경로 검증 예시
	- Linux 컨테이너: docker exec cc_backend_local sh -lc "grep -o '/api/users/vip-only' current_openapi.json || true"
	- Windows 호스트(대안): docker exec cc_backend_local cmd /c type current_openapi.json | findstr "/api/users/vip-only"
	- 기대: '/api/users/vip-only' 1회 이상 출력되면 OK
