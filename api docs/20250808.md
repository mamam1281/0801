...
### 2025-08-20 – user02 완전 삭제 정책 반영 & 개발 도구 업데이트

- 변경 요약
	- 기본 시드 스크립트(`seed_basic_accounts.py`)에서 user002(유저02) 생성 제외 (정책적 제거 요청 반영). 환경변수 `INCLUDE_USER02=1` 설정 시에만 임시 포함 가능(로컬 디버그 용).
	- Dev 전용 endpoint `/api/dev/reset_user02` 에 `delete=true` 쿼리 파라미터 추가 → user id=2 및 연관 action/reward/session 레코드 완전 제거 지원.
	- 기존 reset 동작은 유지(명시적 delete=false 사용 시)하되, 기본 운영 플로우에서는 user02 제거가 표준.

- 검증
	- docker compose exec backend python -m app.scripts.seed_basic_accounts 실행 후 users 테이블 user002 미생성 확인.
	- `/api/dev/reset_user02?delete=true` 호출 200 → {action: deleted} 응답 및 재호출시 idempotent 삭제(no-op) 확인.
	- delete=false 호출 시 (존재하지 않는 경우) 새 user02 생성 → 정책상 테스트 목적 외 사용 지양 문서 명시.

- 다음 단계
	1) CI 파이프라인 사전 단계에서 `?delete=true` 호출로 user02 잔여 데이터 보장 제거.
	2) 향후 game/event 로그 백필 스크립트에서 user02 하드코딩 참조 제거 여부 점검.
	3) 필요 시 보안/프라이버시 문서에 샘플 계정 제거 프로세스 섹션 추가.
