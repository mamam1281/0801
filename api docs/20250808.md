### 2025-08-23 경고 정리(Pydantic v2/httpx)
- 변경 요약: `chat_schemas.AIAssistantResponse`에 `model_config = ConfigDict(from_attributes=True, protected_namespaces=())` 적용해 `model_name/model_version` 보호 네임스페이스 경고 제거. `backend/temp_config.py`를 `SettingsConfigDict` 기반 `model_config`로 전환. 테스트 경고 소거를 위해 httpx `app` 단축키 deprecation 경고를 pytest.ini 필터에 추가.
- 검증: 제한 패키지/프로모, 멱등/홀드/구매 스위트 모두 PASS. Alembic head 단일 유지, /health OK, OpenAPI 스키마 영향 없음.
- 다음 단계: 남은 `class Config` 사용 지점 점검 및 `ConfigDict` 통일. httpx 단축키 간접 사용처가 있다면 `ASGITransport` 명시 전환 검토.

### 2025-08-23 shop_transactions 섀도우(PR1)
- 변경 요약: `shop_transactions_shadow` 테이블 생성, 유니크 제약/인덱스 구성, DB 트리거로 더블라이트 구현.
- 검증: Alembic 단일 head, /health OK, 파리티 쿼리(행/합계) 정상, 데모 삽입 복제 `t` 확인, 구매/한정 패키지 테스트 그린.
- 다음 단계: 운영 데이터 백필 반복 → 컷오버 리비전(PR2) 작성/적용 → OpenAPI 재수출 필요성 점검.
...
- 변경 요약
	- 응답에 last_30d_active_days, lifetime_active_days 추가.
	- 산출 기준: UTC 00:00 경계에서 일 단위 distinct.
	- 소스: 1차 UserAction.created_at, 예외적으로 비어있을 시 GameHistory.created_at으로 보정.
	- 프론트 매핑: 게임횟수는 stats.total_games_played만 사용, 접속일수는 새 필드 중 하나만 사용(권장: last_30d_active_days).
- 검증
	- 컨테이너: pytest -q app/tests/test_user_stats_active_days.py 단독 실행.
	- 슬롯 1회 스핀 후 /api/users/stats 응답의 last_30d_active_days >= 1 확인.
	- OpenAPI 재수출 후 UserStatsResponse에 필드/설명 반영 확인.
- 다음 단계
	- 프론트 UI 라벨에 “UTC 기준” 주석 표기.
	- 대시보드에 7/30/90d 활성일수 추이 차트 추가.
	- UserAction 로깅 100% 전환 시 GameHistory 폴백 제거 검토.
...
### 2025-08-23 – 프론트 운영 환경 고정(ENV 요구 + 캐시/모의 차단)

- 변경 요약
	- 프로덕션(NODE_ENV=production)에서 다음 변수가 필수: `NEXT_PUBLIC_API_BASE`(WS/폴링), `NEXT_PUBLIC_API_ORIGIN`(HTTP).
	- 운영 빌드에서 localStorage 기반 사용자 캐시(`game-user`)는 무시 및 자동 정리. 데이터 소스는 API 응답만 사용.
	- 모의 데이터/시드 UI는 운영에서 비활성. 개발 모드에서만 `NEXT_PUBLIC_ENABLE_MOCKS=1`로 허용.
- 검증
	- 새 세션에서 /api/auth/me, /api/users/stats 응답이 초기화 경로로 사용되는지 확인.
	- 브라우저 콘솔 초기 로그에서 BASE/ORIGIN 설정 값 확인. 누락 시 빌드/런타임이 즉시 오류로 중단되는지 확인.
- 다음 단계
	- README/환경 샘플(.env.production.sample)에도 두 변수를 명시.
	- Frontend CI에서 prod 모드 빌드 시 두 변수 누락을 실패로 처리하는 체크 추가.
...
4. 이벤트 mini-smoke 추가: 이벤트 목록→join→(progress 시도)→claim 흐름·UserReward(EVENT*) 존재 여부 확인. 이벤트 부재 시 xfail.
 - 후속 마이그레이션 적용: shop_transactions PK 인덱스명 `shop_transactions_pkey`로 정규화(리비전 c6a1b5e2e2b1), 백업 테이블 shop_transactions_old 유지

검증
 - 제한/멱등 핵심 테스트 재검증 green: limited holds/concurrency, shop_buy, limited packages/promos
