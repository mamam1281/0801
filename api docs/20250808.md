...
### 2025-08-25 – /api/auth/register 세션 생성 기록 및 단일 스펙 검증

- 변경 요약: 임시 회원가입 엔드포인트(`/api/auth/register`)에서 액세스 토큰 발급 직후 `AuthService.create_session(...)`을 호출하여 세션을 기록. 세션 기반 검증(동시세션/블랙리스트) 경로에서도 신규 토큰이 즉시 유효로 인식되도록 보강.
- 검증 결과: 컨테이너 Playwright 단일 스펙(auth_migration) 포함 21 passed / 2 skipped 확인. 백엔드 제한 테스트 스위트는 모두 GREEN.
- 다음 단계: 나머지 E2E 스펙 점진 복원, 실패 시 WS 하이드레이트/리프레시 경계 재조정. 필요 시 OpenAPI 재수출.
