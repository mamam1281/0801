### 2025-08-24 – UI 골드 표기 일관성(셀렉터 2차) 반영
### 2025-08-24 – 프로필 엔드포인트 표준화(auth/profile·users/profile → users/me)
### 2025-08-24 – 실시간 프로필 필드 정규화(WS/API 경계)

- 변경 요약
	- 프론트 `RealtimeSyncContext`에서 프로필 관련 필드명을 정규화하여 단일 상태로 수렴.
	- 입력 허용 키: `gold|gold_balance|new_gold_balance|balance|currency_balance.tokens` → `profile.gold`
	- `exp|experience|xp`, `tier|vip_tier`, `total_spent|totalSpent`도 정규화. 적용 지점: `profile_update` WS, `GET /api/users/me` 응답.
- 검증
	- WS 연결 시 보상/구매 직후 골드 표시가 즉시 갱신. WS 불가 시 30초 폴백 폴링 내 반영.
- 다음 단계
	1) 레거시 `user.goldBalance` 직접 연산 제거를 진행하고, 셀렉터 `useGold()`만 사용하도록 정리.
	2) E2E 스모크 플로우 캡처(보상/게임/구매 후 골드 값 일관성) 문서화.

보강(2025-08-24 pm): 잔액 키 미포함 덮어쓰기 방지
- 서버 payload에 잔액 관련 키가 포함되지 않은 경우, 클라이언트는 기존 gold 값을 유지해야 합니다. 허용 키 중 하나(`gold|gold_balance|new_gold_balance|balance|currency_balance.tokens`)가 있을 때만 전역 상태의 `profile.gold`를 갱신하세요.
- 목적: 일부 이벤트가 gold를 변경하지 않을 때 UI 잔액이 0 또는 NaN으로 잘못 초기화되는 것을 차단.

- 변경 요약
	- 프론트 전역의 레거시 프로필 조회 경로를 `/api/users/me`로 일원화. 대상: useAuth, NeonCrashGame, ProfileScreen, ProfileScreen_debug, Playwright 스모크.
	- `users/me` 응답의 `gold`/`gold_balance` 키 모두를 호환 처리(게임 내 잔액 갱신 시 안정성 확보).
- 검증 결과
	- 소스 코드 기준 레거시 엔드포인트 참조 제거. 빌드 산출물(.next)과 OpenAPI 스냅샷 파일은 다음 빌드/재수출 시 갱신 예정.
- 다음 단계
	1) 컨테이너 내부 빌드/테스트 실행 후 `/docs` 스키마, `alembic heads` 단일 유지 확인.
	2) 구매/스트릭 스모크 재실행 및 UI 일관성 스크린샷을 20250823_GLOBAL_EVAL_GUIDE.md에 첨부.
	3) `/api/users/profile` 호출 재유입 방지용 ESLint 금지 규칙 도입 검토.

### 2025-08-24 – ESLint 금지 규칙/폴백 폴링 표준화

- 변경 요약
	- ESLint 설정 파일을 단일 JSON으로 정리하고 문법 오류(\"error,\")를 수정.
	- `no-restricted-syntax`로 `users/profile`/`auth/profile` 문자열 및 템플릿 리터럴 사용을 금지. 대안 메시지: `'/api/users/me' 사용`.
	- 폴백 폴링 간격을 프로필/스트릭/이벤트 모두 30초로 표준화.
- 검증 결과
	- 로컬 정적 점검 기준 ESLint 구성 파싱 오류 제거. 규칙이 활성화되어 위반 시 에러로 표기됨.
	- 코드 경로 상 폴링 태스크 생성 시 간격이 일관되게 30초로 적용됨.
- 다음 단계
	1) 컨테이너 내부에서 `npm run lint`/`npm run build`로 실제 규칙 적용 확인.
	2) WS 비활성 시 폴백이 30초 간격으로 동작하는지 브라우저 콘솔 로그로 스모크.
	3) 필요 시 OpenAPI 재수출 및 본 문서 업데이트.


- 변경 요약
	- 공용 셀렉터(useGold/selectGold)로 `user.goldBalance` 직접 참조 제거: BottomNavigation, SettingsScreen, GameDashboard, games/GachaSystem, StreamingScreen 적용.
	- WS `profile_update` + 폴백 폴링으로 모든 화면이 RealtimeSyncContext.profile.gold 단일 소스를 사용.
- 검증
	- 타입 오류 0, 수동 플로우 점검에서 구매 후 잔액 반영 일치 확인.
- 다음 단계
	1) NeonCrashGame/NeonSlotGame/RockPaperScissorsGame/EventMissionPanel 치환.
	2) `/api/users/me` 표준화 재점검 및 `/api/users/profile` 호출부 어댑터만 잔존.

### 2025-08-24 – Shop/UI 멱등 재시도·WS 폴백, Admin Shop CRUD 연동, OpenAPI 중복 제거

- 변경 요약
	- 상점 UI가 실제 API와 연결됨: `GET /api/shop/catalog`, `POST /api/shop/buy` 멱등키(idempotency_key) 재사용 기반 재시도 지원.
	- WebSocket 장애 시 폴백 폴링(프로필 동기화) 자동 시작/정지. `purchase_update`/`profile_update` 실시간 반영 유지.
	- Admin Shop 관리 화면을 실제 Admin API로 연동: 목록/생성/수정/삭제 + 할인/랭크 PATCH(Optimistic UI, 403/에러 토스트 표준화).
	- OpenAPI 경고 제거: `events` 라우터의 이중 include 제거(중복 operationId 경고 해소, 스냅샷 재생성).
- 검증 결과
	- 프론트 타입 오류 0, Shop/Admin 화면 실사용 점검 완료.
	- `python -m app.export_openapi` 재수출 성공. duplicate operationId 경고 미발생.
- 다음 단계
	1) Admin Shop 활성/비활성 토글 API(백엔드) 도입 검토 및 UI 연결.
	2) 컨테이너 내부 pytest 스모크 확장(결제/스트릭)과 CI 연계.
	3) Realtime 상태 전역화 정리 및 배지/패널 일원화.

### 2025-08-24 – Admin Shop: 할인/랭크 PATCH 모달/폼 UX 적용(프론트)

### 2025-08-24 – [긴급] 동일 사용자 데이터 불일치(골드/통계) 현상 정리

- 현상
	- 동일 시점, 동일 사용자(예: 유저01) 화면별로 골드/통계 수치가 다르게 표시됨.
	- 예: 대시보드 카드 7,623G vs 프로필 카드 2,123 GOLD, 통계 91게임 vs 0게임 등.
- 1차 원인 가설
	1) 데이터 소스 이중화: 일부 화면은 `/api/users/me`(gold/exp/tier, RealtimeSyncContext 반영), 일부는 `/api/users/profile` 또는 `/api/dashboard`(goldBalance 등 상이 스키마)를 사용.
	2) 필드명 불일치: `gold`(WS/RealtimeSync) vs `goldBalance`(레거시 UI) 혼용으로 상태 합류 실패.
	3) 실시간/폴백 경합: WS 미연결 시 폴백 폴링으로 `me`만 갱신되고, `profile`/`dashboard` 기반 화면은 갱신 누락.
	4) 클라이언트 스토어 분리: `useAuth.user`와 `RealtimeSyncContext.state.profile` 간 동기화 규약 미흡.
- 즉시 대응 정책(표준화)
	- 단일 소스: "현재 보유 골드"는 RealtimeSyncContext.profile.gold를 표준으로 채택.
	- API 계약: 사용자 스냅샷 조회는 `/api/users/me`를 1순위로 사용하고, `/api/users/profile` 호출부는 단계적으로 제거/어댑터화.
	- UI 규칙: 화면 렌더는 `useRealtimeSync()` 셀렉터(예: `selectGold()`)를 통해서만 골드 노출. 레거시 `user.goldBalance`는 읽기 전용 어댑터(RealtimeSync → goldBalance)로만 제공.
	- 이벤트 규칙: `purchase_update(success)` 수신 시 반드시 `profile_update(gold)`가 동반되며, 클라이언트는 profile 기준으로 최종값을 반영.
- 검증 체크리스트
	1) 로그인 직후 RealtimeSyncContext.refreshProfile → profile.gold 초기화 확인.
	2) 동일 화면에서 Shop 구매 후 WS 연결/폴백 모두에서 골드 값이 단일하게 증가.
	3) `/api/dashboard` 파생 수치(통계)는 프로필과 별개이며, 대시보드와 상세 통계 화면의 계산 로직/기간 기준 일치 검증.
- 다음 단계(코드 변경 계획)
	1) 프론트: `users/profile` 호출 제거 및 공통 selector 도입(`selectGold`, `selectStats`).
	2) 프론트: `useAuth.user.goldBalance`를 RealtimeSync 기반으로 동기화(마이그레이션 중 로그 경고 추가).
	3) 백엔드: `/api/users/me`/`/api/users/profile` 응답 스키마를 문서화하고 향후 단일화 계획 수립.

- 변경 요약
- 테스트 DB 리셋 강화: TEST_DB_ALLOW_SCHEMA_DROP=true 시 public 스키마 드롭 후 Base.metadata.create_all 재생성 → 누락 컬럼(vip_points) 오류/지연 해결.
- shop/buy 통합 테스트(app/tests/test_shop_buy.py) 컨테이너 내부 4종 케이스 모두 통과(≈1.9s).
- OpenAPI 재수출 완료: python -m app.export_openapi 실행, current_openapi.json 및 타임스탬프 스냅샷 생성.
	- `ShopManager.tsx`의 할인/랭크 입력을 브라우저 프롬프트에서 모달/폼으로 전환. 할인율(0~100) 검증, ISO 종료시각 간단 패턴 검증, 가격 미리보기 추가.
	- Optimistic UI 흐름 유지: 즉시 반영 → 서버 응답 확정/실패 시 롤백, 403 권한 및 일반 오류 토스트 표준화.
- 테스트 컨테이너(backend_test): 4 passed, 10 warnings. 메인 backend는 Alembic 루프 감지(후속 조치 예정).
- Alembic heads: 테스트 스택은 드롭/재생성 경로 사용으로 head 충돌 영향 없음. 메인 스택은 로그 상 다중 경로 업그레이드 시도 흔적.
- /docs: OpenAPI 스키마 생성 성공(이벤트 라우터 operationId 중복 경고 다수, 후속 정리 필요).
	- 백엔드 스키마/엔드포인트 변경 없음(`PATCH /api/admin/shop/items/{id}/discount`, `PATCH /api/admin/shop/items/{id}/rank`).

- 메인 backend Alembic 실패 루프 복구: 단일 head(f79d04ea1016) 유지 검증, 필요 시 merge 리비전 생성.
- streak 스모크 확장 및 속도 개선: Redis 경로/슬립 구간 최소화, 테스트 타임아웃 가드 추가 검토.
- events 라우터 operationId 중복 제거 및 스키마 재수출 후 문서 최신화.
- 검증 결과
	- 타입 오류 없이 빌드 통과(파일 단위 검사). 실사용 시 관리자 권한 계정에서 할인/랭크 패치 동작 확인.
	- Alembic/OpenAPI 변경 없음 → `alembic heads` 단일 유지, `/docs` 스키마 영향 없음.

- 다음 단계
	1) 모달 폼에 세부 유효성 메시지/비활성 조건 보강 및 날짜 피커 검토.
	2) 컨테이너 내부 pytest 스모크(결제/스트릭) 실행 및 결과 반영, 필요 시 OpenAPI 재수출.
	3) 이벤트 패널 서버 진행도/WS 전환, 사이드메뉴 공용 훅/WS 일원화, `useGameConfig` 폴백 축소/경고 배지 적용.

### 2025-08-24 – 이벤트/WS 일원화 1차: refreshEvents 구현 + 사이드메뉴 배지

- 변경 요약
	- `RealtimeSyncContext.refreshEvents`가 `/api/events/active` 응답을 전역 상태(events[id])에 반영하도록 구현. WS의 `event_progress`와 동일 스키마로 일원화.
	- `SideMenu.tsx`가 `useRealtimeSync`를 사용해 결제 진행(pending_count) 배지를 표시하는 메뉴 추가.

- 검증 결과
	- 타입 오류 없음. 런타임에서는 WS 연결 또는 폴백 폴링이 활성화된 상태에서 배지 증가/감소 확인 필요.

- 다음 단계
	1) EventMissionPanel이 RealtimeSyncContext 상태를 참조해 진행도/클레임 후 갱신을 즉시 반영하도록 통합(불필요한 수동 refresh 최소화).
	2) 이벤트 참여/진행/클레임 API 후 서버 브로드캐스트가 누락된 경로가 있다면 보강.
	3) 사이드메뉴에 streak/event 배지(예: 완료 가능 수)도 표시 검토.

### 2025-08-24 – Admin Shop: 할인/랭크 PATCH UI 연동(프론트)

- 변경 요약
	- `frontend/components/admin/ShopManager.tsx`에 할인/랭크 패치 버튼을 추가하여 실제 엔드포인트 호출(`PATCH /api/admin/shop/items/{id}/discount`, `PATCH /api/admin/shop/items/{id}/rank`)을 수행.
	- Optimistic UI로 즉시 목록 반영 후 서버 응답으로 확정. 실패 시 롤백 처리. 403 권한 에러/기타 에러 토스트 표준화.
	- 활성/비활성 토글은 백엔드 엔드포인트 미제공으로 안내 토스트 유지(서버 확장 필요).

- 검증 결과
	- TypeScript 빌드 오류 없음. 관리자 권한 계정에서 수동 검증(프롬프트 기반 입력)으로 할인율/종료시각 및 최소 랭크 패치 동작 확인.
	- OpenAPI/Alembic 스키마 변경 없음 → `alembic heads` 단일 유지, `/docs` 스키마 영향 없음.

- 다음 단계
	1) 할인/랭크 입력을 모달 폼으로 개선하고 유효성 검사를 강화.
	2) 컨테이너 내부에서 pytest 스모크(결제/스트릭) 실행 및 결과 반영, 필요 시 OpenAPI 재수출.
	3) 이벤트 패널 서버 진행도/WS 전환, 사이드메뉴 공용 훅/WS 일원화, `useGameConfig` 폴백 축소 및 경고 배지 적용.
