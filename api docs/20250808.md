...
### 2025-08-16 – user_actions.created_at 마이그레이션 및 가챠 통계 테스트 안정화

- 변경 요약
	- 레거시 스키마(`user_actions` 테이블)에는 `timestamp`만 존재하고 ORM/서비스 코드가 `created_at` 컬럼을 기대하여 가챠 pull 기록 시 `OperationalError: no such column: created_at` 발생.
	- Alembic 리비전 `d1993e6fdab6_add_created_at_to_user_actions_if_missing` 추가: 컬럼 존재 여부 검사 후 없을 때만 `created_at DATETIME` 추가 및 기존 `timestamp` 값으로 백필.
	- 기존 action 기록/인덱스(`ix_user_actions_type_created`, `ix_user_actions_user_created`)가 (created_at | timestamp) 조건 처리하던 이원 상태 해소 방향 마련.
	- 테스트 `test_achievements_stats.py::test_game_stats_increment_after_gacha_pull` 실행 전 사용자 토큰 잔액 선충전(5000 비용 체계 반영) + created_at 컬럼 도입으로 삽입 오류 제거 → PASS 확인.

- 검증 결과
	- 로컬 `inspect_user_actions.py` 실행 전: columns=[id,user_id,action_type,details,timestamp], created_at 없음 확인.
	- 마이그레이션 추가 후 sqlite 환경에서 upgrade 반영 → 새 컬럼 추가 및 값 백필 (재실행시 no-op).
	- 단일 테스트 실행 결과: 1 passed (가챠 통계 rarity_counts 증가 조건 충족) / 경고 4건(Pydantic class Config deprecation 기존과 동일).
	- OpenAPI 스키마 엔드포인트 변화 없음 (내부 action 로깅 컬럼 추가이므로 문서 재수출 불필요 판단).

- 다음 단계
	- user_actions 모델 정의에 created_at ↔ timestamp 병행 존재 시 정리 (timestamp 필드 Deprecation 경고/주석 추가 후 장기 제거 계획 수립).
	- 일반 게임 통계 API(`/api/games/stats/...`) 재활성화 시 action_data / created_at 활용 집계 지표 확장.
	- 남은 Pydantic v2 ConfigDict 전환으로 경고 제거.
