...
- TestClient/httpx 호환 이슈 해결: 컨테이너 내 httpx 0.28.1 → 0.27.0 핀 고정으로 pytest 수집 오류 제거
- Crash 관련 테스트: 엣지케이스 PASS, 라이프사이클 테스트는 토큰 미설정 시 skip (TEST_USER_TOKEN 설정 시 통과 가능)
- OpenAPI 재수출 완료: /api/admin/rewards/grant-bulk, /api/admin/crash/sessions 노출 확인
- Crash DB 테이블 생성: crash_sessions, crash_bets 존재 확인 (타깃 리비전 업그레이드 적용)
- Alembic 다중 head 존재: head 전체 업그레이드는 보류, 당분간 특정 리비전 타깃 업그레이드로 운용
- Config 중복 축소 착수: 유틸 스크립트를 환경변수 기반으로 전환(add_image_url_field.py, app/kafka_client.py)
