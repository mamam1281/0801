...
- 변경 요약: SSR 미들웨어(`frontend/middleware.ts`)에 JWT 토큰 실제 검증 로직 추가(백엔드 `/api/auth/verify` 경량 엔드포인트 호출). 프론트엔드 Dockerfile에 `RUN npm install axios` 명령 추가, 컨테이너 빌드로 SSR 미들웨어 정상 동작 확인.
