...
### 1.5.1 구매 멱등/동시성 개선 (2025-08-17)
변경 요약:
- `/api/shop/buy` 엔드포인트에 Redis 기반 pre-lock (`shop:idemp:lock:*`) + Redis 미가용 환경(테스트/로컬)용 프로세스 내부 `threading.Lock` 폴백 추가 → 동일 `idempotency_key` 동시 다중 요청 시 단일 트랜잭션만 생성.
- Pending → 재시도 시 `extra.gateway_reference` 저장 후 폴링해 `success/failed` 승격.
- Concurrency 테스트(`test_purchase_concurrency_race_single_record`) xfail 제거 및 단일 영수증 보장 검증 통과.

검증 결과:
- Idempotent duplicate (success / failed / pending→success) 시나리오 개별 테스트 PASS.
- Race 단일화 테스트 PASS (fallback lock 경로에서 동작 확인). Redis 실제 연결 환경 재검증 TODO.
- 기타 레거시 shop 테스트 일부(DB 초기화 스크립트 `pg_tables` 의존) 오류 → 테스트 코드 정비 필요.

다음 단계:
1) Alembic UniqueConstraint(user_id, product_id, idempotency_key) 실제 마이그레이션 존재 여부 확인 & 문서 반영.
2) 실패/부족/거절/환불 경로 통합 커버리지 측정 → 60% 커버리지 목표 대비 갭 산출.
3) Redis 연결 환경에서 lock 재검증(로그: 획득/반납) + 모니터링 메트릭 추가(PROMETHEUS counter: purchase_lock_wait / purchase_lock_contention).

### 1.6 테스트 (최소 품질 게이트) ※ (기존 1.7을 상위로 이동: MVP 안정화에서 테스트 우선)
- [🟡] 단위 테스트 핵심(인증, 구매, 게임 보상, 행동 로깅)
	- 구매: 멱등/동시성/게이트웨이 pending→success/always_fail 재사용 PASS
	- 게임 보상/행동 로깅: 부분 PASS (표준 스키마 확정 후 보강 필요)
	- 인증: 기존 테스트 유지 (회전/락아웃 확장 별도)
- [❌] 통합 테스트: 온보딩→슬롯/가챠→구매→잔액 검증 흐름 (작성 미완) 
	- TODO 흐름: signup→slot spin→gacha pull(토큰 차감/rare 보상)→shop buy(pending→success)→최종 balance diff assert
	- 향후 headless 모드 + 기본 seed(카탈로그) 픽스처 추가 필요

### 1.7 프론트 사용자 여정 (순서 조정: 테스트 후속)
- [ ] 랜딩 페이지 (Hero + 초대코드 진입)
- [ ] 초대코드 입력 화면
- [ ] 회원가입/로그인 모달
- [ ] 온보딩 튜토리얼 (2~3 스텝 최소)
