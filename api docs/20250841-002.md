...
- [🟡] 구매 로직 멱등 처리 (/api/shop/buy Redis 기반 멱등 포함, DB unique 제약 미도입) → DB 레벨 idempotency_key UNIQUE 추가 예정
- [🟡] 화폐 시스템: 단일 `cyber_token_balance` 로 Gems/Premium 겸용 사용 중 → `regular_coin_balance`, `premium_gem_balance` 이원화 컬럼 추가 예정 + 서비스 추상화(`CurrencyService`)
- (선택) 한정 패키지 / 프로모션 관리: 기본 구매/할인 계산/예약(hold) 구현 O, 운영 UI/상세 정책 LATER 이동

GAP 요약:
1) 통화 이원화: User 테이블 신규 컬럼 2개 추가 (INT NOT NULL DEFAULT 0)
2) 멱등성 강화를 위한 거래 테이블(idempotency_key UNIQUE) / 중복 재시도 시 DB 자연 충돌 처리
3) CurrencyService: add/deduct + SELECT FOR UPDATE / 음수 방지 예외
4) 테스트: 성공/중복/잔액부족/경쟁(동시) 시나리오 Pytest 추가
