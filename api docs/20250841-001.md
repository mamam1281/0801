...
---
### (2025-08-16) GameHistory 인덱스 & 실시간 브로드캐스트 1차 적용

#### ✅ TODO #2 (최소 인덱스) 상태
- Alembic 리비전 체인 정리: 기존 2개 head(`20250816_add_game_history_and_follow_relations`, `20250815_merge_receipt_unique_and_prev`) → merge 리비전 `20250816_merge_game_history_receipt_heads` 추가로 단일화.
- 신규 인덱스 리비전: `20250816_add_game_history_indexes`
  - 생성 인덱스: `ix_game_history_user_game_created (user_id, game_type, created_at)`
  - 목적: 다음 동시/우선 쿼리 패턴 커버
    * `/api/games/history?game_type=…` (user_id + game_type + 최신순 created_at DESC 스캔)
    * `/api/games/{game_type}/stats` (user_id + game_type 사전 필터 후 집계 비용 감소)
    * `/api/games/profile/stats` (left-prefix user_id + created_at 활용 — game_type 부분 탐색 후 제외 가능)
  - 다른 후보 인덱스 (보류):
    * `(user_id, created_at)` : game_type 다양화/선택도 하락 시점에만 추가
    * `created_at` 단독 : 파티셔닝/아카이빙(예: 월별) 설계 필요 시점

#### 변경 요약
| 항목 | 리비전/파일 | 설명 |
|------|-------------|------|
| Merge heads | 20250816_merge_game_history_receipt_heads | 2개 동시 head 병합 (스키마 NO-OP) |
| Composite Index | 20250816_add_game_history_indexes | `ix_game_history_user_game_created` 생성 (존재 시 skip) |
| 서비스 브로드캐스트 | app/services/history_service.py | `log_game_history` 커밋 후 WS 비동기 broadcast create_task 추가 |

#### 실시간(WebSocket) 연계
- `app/main.py` 내 `broadcast_game_history_event` 활용.
- `log_game_history` 내부에서 레코드 커밋 후 asyncio loop 실행 중일 때 `create_task` 로 non-blocking 전송 스케줄.
- 페이로드: { id, user_id, game_type, action_type, delta_coin, delta_gem, session_id, created_at }
- 예외 처리: 브로드캐스트 실패는 DEBUG 로그로만 기록 (주 트랜잭션 불변성 유지).

#### 검증/안전장치
- 인덱스 생성 전: 테이블/컬럼 존재 여부 + 기존 인덱스 존재 여부 점검.
- 다운그레이드: 존재 시에만 drop (idempotent).
- 순환 import 발생 가능성: `history_service` → `main` (함수 참조) 경로; FastAPI app import 순서 상 런타임에 함수 로드 후 사용되므로 문제 없음. 필요 시 DI/이벤트버스 추상화 고려.

#### 다음 즉시 단계 제안
1. 컨테이너 내 `alembic upgrade head` 실행 후 `alembic heads` 로 단일 head 재확인.
2. 소량 샘플 데이터 삽입 후 `/api/games/history` 쿼리 플랜 확인(`EXPLAIN ANALYZE`) — 인덱스 사용(BTREE) 여부 스냅샷 문서화.
3. 대량(예: 50k rows) synthetic load 스크립트 작성 예정 (row 폭: user_id 분포 1k, game_type 3~5) → 선택도 변화 관찰.
4. 브로드캐스트 구독 FE 데모: `/ws/games` 수신 콘솔 로그 추가(타입 game_history 필터링).
5. 임계(>500k) 도달 시 roll-up/materialized view 설계(일/주 집계) 및 캐시 레이어(Redis) 적용 로드맵 정의.

#### 리스크 & 모니터링
| 리스크 | 영향 | 대응 |
|--------|------|------|
| 순환 import 확장 | 스타트업 시 AttributeError | lazy import 혹은 event dispatcher 도입 준비 |
| 인덱스 과다 조기 추가 | write 성능 저하 | 단일 composite 최소화 전략 유지, growth metrics 기준 재평가 |
| 브로드캐스트 폭주 | WS send 지연 | batch/queue(Deque) + flush 타임슬라이스 후속 적용 고려 |
