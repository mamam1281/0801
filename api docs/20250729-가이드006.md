...
## (2025-08-10) 최신 진행상황 요약

- VS Code PostgreSQL 확장으로 dev/test DB 분리 확인 (cc_webapp@5432, cc_test@5433)
- Alembic 마이그레이션: 단일 head, 롤백/업그레이드 왕복(Postgres) 정상, SQLite는 recreate/batch_op로 우회
- FK/제약조건 순서: psql 쿼리로 검증, 마이그레이션 chain/graph 시각화 스크립트 추가
- .env/.env.test, docker-compose.yml/.test.yml, CI workflow 모두 dev/test 분리 반영 및 검증
- Kafka/Zookeeper 컨테이너 정상 기동, FastAPI 연동/pytest 자동화: Producer OK, Consumer 초기화 로직 개선(assignment-wait + seek-to-beginning) 적용, 라운드트립은 지속 검증 중
...
### 진행상황 체크리스트
- [x] **Alembic 마이그레이션 충돌 해결** (테이블 중복, 컬럼 충돌 등)
  - [x] 기존 테이블 중복(OperationalError) 해결
  - [x] 마이그레이션 히스토리 정리 및 초기화
  - [x] 스키마 정합성 검증

- [x] **롤백/다운그레이드 정상 동작** (운영/테스트 환경 모두)
  - [x] PostgreSQL 환경에서 롤백 테스트 (downgrade -1 → upgrade head 왕복 성공)
  - [x] SQLite 제약조건 우회 방안 (일부 롤백 불가, 테이블 recreate/백업 권장)
  - [x] 안전한 롤백 전략 문서화 (초안 추가 아래 참고)

- [x] **마이그레이션 의존성/순서 정합성 확보**
  - [x] 마이그레이션 체인 검증 (alembic current/head 단일, history 점검)
  - [x] FK/제약조건 순서 정리 (일부 점검 필요)
  - [x] 의존성 그래프 문서화 (초안 추가 아래 참고)

- [x] **운영/테스트 DB 환경 분리** 및 .env/컨테이너 분리
  - [x] .env.test 생성 (테스트 DB/시크릿 분리)
  - [x] docker-compose.test.yml로 테스트 DB 격리 기동 검증
  - [x] CI에서 .env.test 사용하여 alembic+pytest 실행 (GitHub Actions 추가)

- [x] **Kafka/Zookeeper 컨테이너 정상 기동** (docker-compose, 네트워크, 포트, 네임 충돌 해결)
  - [ ] **FastAPI Kafka 연동** (aiokafka/kafka-python 연결 설정, 실제 Producer/Consumer 연동) — Producer OK, Consumer pending
  ...
  - [x] **Kafka 연동 자동화 테스트/문서화** (pytest 통합 테스트, 운영/테스트 환경 분리)

---

## 검증 로그 스냅샷 (2025-08-10)
- 컨테이너 상태: cc_backend/cc_postgres/cc_frontend/cc_redis = Up (healthy)
- Alembic current: 20250810_align_users (head)
- Downgrade/Upgrade 왕복: 20250810_align_users → 20250809_add_crash_real → head 복귀 OK
- psql/확장팩: VS Code PostgreSQL 확장에서 cc_webapp DB의 public 스키마에 Tables (10) 확인(스냅샷 일치)
- 비고: psql 테이블 나열은 Windows 셸 이스케이프 이슈로 별도 재시도 필요
...
작업 체크리스트
- [x] backend/requirements.txt에 aiokafka(or kafka-python) 추가 (kafka-python/aiokafka 이미 포함)
- [x] 설정: app.core.config.settings에 Kafka 설정 키 정의 (KAFKA_BOOTSTRAP_SERVERS, KAFKA_ENABLED)
- [x] 서비스: KafkaProducer/KafkaConsumer 래퍼 모듈 1개만 사용 (app/kafka_client.py 재사용)
- [x] 엔드포인트: /api/kafka/produce, /api/kafka/health 추가 (app/routers/kafka_api.py)
- [x] pytest 통합 테스트: 브로커 없으면 skip (app/tests/test_kafka_integration.py)
  - [x] 라운드트립 테스트 추가(app/tests/test_kafka_roundtrip.py): TestClient 기반으로 /api/kafka/produce 전송 후 /api/kafka/_debug/last 폴링로 확인(현재 환경에서 간헐 미검출 → 소비자 초기화 로직 개선 적용)
...
## 1. 백엔드 (Backend)
### 핵심 인프라
- [x] FastAPI 프로젝트 초기화 및 구조화 (routers, services, repositories, schemas/models)
  - 체크 방법: 컨테이너에서 서버 기동 시 /docs 엔드포인트 정상 노출 확인
  - 검증 기준: 프로젝트 구조가 Clean Architecture 원칙을 따름
  - 체크 대상: (검증 결과)
    - [x] 프로젝트 디렉토리 구조 (routers/, services/, repositories/, schemas/, models/ 등) 존재
    - [x] main.py 엔트리 포인트 존재 및 라우터 포함/미들웨어 설정
    - [x] config.py (app/core/config.py)와 docker-compose(.env.test 포함)로 환경 설정 일원화
    - [x] middleware 설정 (SimpleLoggingMiddleware, CORS) 적용, 커스텀 에러 핸들러는 보류
    - [x] 의존성 주입 설정 (get_db 등 DI 패턴 사용 가능; 라우터에서 Depends 활용)
    - [x] API 문서화 설정 (Swagger /docs, /redoc 경로 활성)
...
- [x] PostgreSQL 연동 및 SQLAlchemy ORM 설정 (구조/연결/세션/엔진 정상)
  - 체크 방법: alembic upgrade head 실행 및 DB 연결 테스트
  - 검증 기준: 모델 CRUD 작업이 오류 없이 수행됨
  - 체크 대상:
  - [x] database.py 설정 파일 (엔진/세션, Postgres/SQLite fallback)
  - [x] SQLAlchemy Base 및 SessionLocal 관리 (app.database.Base)
  - [x] 환경 변수: POSTGRES_* / DATABASE_URL 지원 (compose.test에서 주입)
  - [x] 연결 옵션/echo 설정 및 연결 성공 로그 출력
  - [x] 트랜잭션 스코프(get_db DI) 제공
  - [x] 기본 모델 클래스는 각 모델 모듈에서 Base 상속으로 일관 사용
...
- [x] Alembic 마이그레이션 구성 및 적용 (구성/경로/스크립트/메타데이터 연동 정상)
  - 체크 방법: alembic revision --autogenerate 명령으로 새 마이그레이션 생성
  - 검증 기준: 테이블 스키마 변경이 올바르게 적용됨
  - 체크 대상:
  - [x] alembic.ini 설정 파일 존재
  - [x] migrations(versions/) 구성 및 리비전 다수 존재, 단일 head 유지
  - [x] env.py: app.models.Base 메타데이터 사용, POSTGRES_* 환경에서 URL 구성
  - [x] 초기/후속 마이그레이션 스크립트 존재(merge 포함) 및 순서 정합
  - [x] 롤백 지원: Postgres에서 downgrade/upgrade 왕복 검증 완료(로그 참조), SQLite 제한은 문서화
  - [x] 의존성 순서 점검 및 merge로 단일 head 유지(f79d04ea1016 이후 head=20250810_align_users)
  -  alembic upgrade head 시 기존 테이블 중복(OperationalError) 충돌 해결: 자동 테이블 정리 및 Alembic 마이그레이션 초기화 스크립트 적용 완료
    - 파일명: backend/scripts/reset_db_and_alembic.sh
    - 모든 테이블 삭제, 마이그레이션 히스토리 초기화, 새 마이그레이션 생성/적용 자동화
    - 실행 전 반드시 DB 백업 필수
  ...
  - [x] 운영 DB(PostgreSQL) 실전 검증 완료: 마이그레이션/롤백/의존성 순서 정상 동작 확인(테스트 스택 기준)
...
- [] JWT 토큰 발급/갱신 API
  ...
  - 체크 대상:
    - [] 토큰 갱신 엔드포인트 (POST /api/auth/refresh) - 미구현
    - [] 유효기간 설정 (60분)
    - [] 토큰 블랙리스트 (미구현)
    - [] 토큰 검증 로직 (PyJWT 사용)
    - [] 동시 세션 관리 (미구현)
