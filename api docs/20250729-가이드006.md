## 초대코드 리포지토리 및 테스트 요약 (2025-08-11)

- InviteCodeRepository 도입: '5858'은 무제한 유효, 그 외 코드는 단일 사용(is_active & is_used 기반)으로 검증/소비 처리.
- 신규 단위 테스트 추가: 5858 무제한, 단일 사용 코드 validate→consume, 사용됨/비활성 코드 거부. 로컬에서 3 passed 확인.
- 인증 라우터 연동: /api/auth/signup, /api/auth/check-invite/{code}가 리포지토리 사용. signup 시 유효 코드 소모(5858 제외).
- 경고 상태: pydantic config deprecation 일부 잔존, httpx TestClient app shortcut 및 passlib crypt 경고는 CI에서 필터링 적용.
- Admin 보조: /api/admin/invite-codes(GET) 목록 조회, /api/admin/invite-codes(POST) 코드 시드 기능 추가.
...
- [✅] FastAPI 프로젝트 초기화 및 구조화 (routers, services, repositories, schemas/models)
  ...
  - [✅] middleware 설정 (SimpleLoggingMiddleware, CORS) 적용, 전역 예외 핸들러(app.core.exceptions) 활성
...
❌ 스키마 레지스트리 연동 (필요시)
 ...
 - [✅] JWT 인증 및 권한 관리 (초대코드/닉네임 기반, VIP/등급별 접근제어)
  ...
  - 체크 대상:
   - [✅] JWT 인코딩/디코딩 기능
   - [✅] 토큰 발급 엔드포인트
   - [✅] 만료 시간 설정 (기본 30분, JWT_EXPIRE_MINUTES로 조정)
   - [✅] 토큰 리프레시 메커니즘 (POST /api/auth/refresh 기본 구현; 회전/블랙리스트는 추후)
   - [✅] 역할 기반 접근 제어 (RBAC)
...
### 사용자 여정 1: 진입 및 등록 (2025-08-11 업데이트)
- [x] 초대코드 유효성 검증 API (GET /api/auth/check-invite/{code})
  - 5858 코드: 항상 valid=true, infinite=true로 응답 (무한 재사용)
  - 그 외 코드는 현재 valid=false, reason=UNKNOWN_OR_UNSUPPORTED_CODE
  - 검증: 컨테이너 내 pytest 스모크 통과(app/tests/test_invite_signup_login_me.py)

- [x] 회원가입 API (POST /api/auth/signup)
  - 유효한 초대코드(5858) 기반 가입, bcrypt 해싱 사용, 기본 설정 적용
  - 성공 시 JWT 발급 및 사용자 응답 반환
  - 검증: signup→login→me 플로우 테스트 통과
  - 주: 5858은 사용 처리 없이 무한 재사용 정책(요구사항 반영)

- [x] 로그인 API (POST /api/auth/login)
  - 올바른 자격 증명 시 200 + JWT 발급, 실패 시 401
  - 최근 로그인 시간 업데이트 포함
  - [✅] 액세스 토큰 발급 (HS256, 기본 30분 만료 — JWT_EXPIRE_MINUTES로 조정)
  - [✅] 리프레시 토큰 발급/검증 경로(기본) — 회전/블랙리스트는 추후
  - [x] 사용자 정보 응답 (GET /api/auth/me)
    - 검증: Bearer로 /api/auth/me 200 및 정확한 사용자 정보 반환 확인

- [x] JWT 토큰 발급/갱신 API
  - 체크 방법: 만료된 토큰으로 갱신 요청, 유효한 토큰으로 보호된 API 접근
  - 검증 기준: 토큰 만료/갱신 메커니즘이 정상 작동 
  - 체크 대상:
  - [✅] 토큰 갱신 엔드포인트 (POST /api/auth/refresh) — 구현 완료
  - [✅] 유효기간 설정 (기본 30분; 환경변수로 조정 가능)
    - [] 토큰 블랙리스트 (미구현)
  - [✅] 토큰 검증 로직 (python-jose 사용)
    - [] 동시 세션 관리 (미구현)
    - [] 강제 로그아웃 기능 (미구현)
...
- [x] 사용자 정보 조회 API (GET /api/auth/me)
...
## (2025-08-11) 변경 요약/검증/다음 단계

변경 요약
- InviteCodeRepository 도입 및 인증 라우터 연동: '5858' 무제한, 일반 코드는 단일 사용 검증/소비. Admin 초대코드 목록/시드 엔드포인트 추가.
- 리프레시 플로우 안정화: HTTPBearer(auto_error=False) 적용으로 본문 토큰만으로도 /api/auth/refresh 처리.
- 테스트/CI: 초대코드 리포지토리 단위테스트 포함 리그레션 10개 통과; httpx/passlib 경고는 pytest 필터 적용. Kafka 스모크는 비차단 모드 유지.
- 문서/OpenAPI: 스키마 재수출 경로 유지(current_openapi.json). 중복 소스 제거 정책 재확인.

검증 결과
- pytest curated bundle: 10 passed (signup→login→me, refresh header/body, invite repo). 경고는 기능 영향 없음.
- /health 200, /docs 정상 노출, 응답 헤더 X-Request-ID 확인.
- Alembic heads: 단일 head 유지(f79d04ea1016 체인 기준). 업/다운 왕복 스팟 체크 OK.

다음 단계
- Pydantic v2 마이그레이션으로 class-based config deprecation 제거, httpx 버전 고정 재확인.
- 초대코드 관리 고도화: 사용 이력 조회/검색, 만료/비활성 정책, CSV 벌크 업서트.
- OpenAPI 재수출 자동화 크론/CI 스텝 추가 및 변경 요약 자동 삽입 스크립트 구상.
...
검증 결과
- 컨테이너 내부:
  - `alembic heads` → 단일 head 유지(20250810_align_users)
...
- Alembic: heads → 20250811_core_ix 단일 유지. alembic upgrade head 성공(스키마 인식 로직으로 컬럼 부재 오류 해소).
