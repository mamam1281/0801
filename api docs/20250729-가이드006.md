...
### 다음 단계(2025-08-11)
- [ ] app/tests/test_invite_validate.py 작성: 유효/만료/사용초과/미존재 코드 케이스 + 응답 포맷 검증
- [ ] InviteCode 모델↔DB 스키마 정합성 확인 및 수정: expires_at/max_uses/used_count 필드 재검증(필요 시 마이그레이션 보강)
- [ ] 라우터 최소 등급 의존성(require_min_rank) 구현 및 대표 엔드포인트 적용(+403/200 테스트)
- [ ] 백업/복구 스크립트(ps1) 및 간단 Runbook 섹션 추가
...
- [] JWT 인증 및 권한 관리 (초대코드/닉네임 기반, VIP/등급별 접근제어)
  ...
  - [⚠️] 역할 기반 접근 제어 (RBAC) — 라우터에 관리자 요구 엔드포인트 존재(/api/invite/* 일부)하나 테스트 스텁은 비어 있음(test_auth_rbac.py empty)
  - [⚠️] VIP/PREMIUM/STANDARD 등급별 권한 — 서비스에 rank 체크 유틸 있음(check_rank_access)이나 적용/테스트 보강 필요
...
- [] 초대코드 유효성 검증 API (/api/invite/validate)
  ...
  - 체크 대상: 코드는 고정값 5858 ✅ 테스트 완료 - 무한재사용 구현됨
    - [⚠️] 유효성 검사 로직 (5858: valid=true) — 코드상 구현 확인, app/tests 하 실제 테스트 보강 필요
    - [⚠️] 만료 시간 검사 — 모델/서비스에 로직 존재, E2E 테스트 부재
    - [⚠️] 사용 횟수 제한 검사 — 현재 기본 정책: 무한재사용(기본값 5858), 개별 코드 제한 필드 존재 여부는 마이그레이션/모델 재확인 필요
    - [ ] 코드 보안 (예측 불가능성) — DEFAULT_INVITE_CODE 환경변수 난수화 권장, 운영 시 고정값 금지
    - [⚠️] 응답 포맷 (is_valid/error_message/code/expires_at/remaining_uses) — OpenAPI와 일치 여부 테스트 필요
...
- [✅] 회원가입 API (POST /api/auth/signup)
  ...
  - [✅] 비밀번호 해싱(bcrypt)
  - [⚠️] 초기 사용자 설정 (기본 등급/VIP/토큰 초기값 등 일부 값은 모델/서비스에 기본값, 세부 정책 합의 필요)
  - [✅] 회원가입 후 토큰 발급
...
- [✅] 로그인 API (POST /api/auth/login)
  ...
  - [ ] 리프레시 토큰 저장소 (DB) — 현재는 즉시 재발급 전략, 저장소 미구현
