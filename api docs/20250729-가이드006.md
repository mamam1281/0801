...
다음 단계
- [완료] 상품 카탈로그/가격 산정 API 추가: GET /api/shop/products, GET /api/shop/price/{product_id}
- [완료] 내 거래 내역 조회 API 추가: GET /api/shop/transactions (토큰 사용자 기준)
- [완료] 서버 가격 산정 사용: POST /api/shop/buy 시 compute_price 우선 적용, 클라이언트 금액 신뢰 안 함
- [완료] 거래 기록(영수증 코드 포함) 저장: ShopTransaction에 성공 케이스 기록
- [완료] 결제 게이트웨이 어댑터(개발용) 추가: PaymentGatewayService (env: PAYMENT_GATEWAY_MODE)
- [완료] 결제 실패/대기 처리: /api/shop/buy(gems) 결제 선처리, failed/pending 트랜잭션 기록 및 메시지 반환
- [완료] 관리자 거래 검색/필터 API: GET /api/admin/transactions (user_id, product_id, status, 기간, receipt_code, limit)
- [완료] 환불 API(관리자): POST /api/admin/transactions/{receipt_code}/refund (성공 케이스에 한해 환불 처리)
- [참고] PAYMENT_GATEWAY_MODE=always_success|always_fail|pending_then_success|random (기본 always_success)
- 컨테이너 내부에서 alembic upgrade head 실행 후 /docs 재확인 및 current_openapi.json 필요 시 재수출
