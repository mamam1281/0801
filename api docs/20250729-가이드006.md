...
- Backend Pydantic v2 마이그레이션: class Config 사용 지점을 model_config = ConfigDict(...)로 전환 완료. BaseSettings는 SettingsConfigDict 적용. 주요 스키마: auth, admin, ai_schemas, chat_schemas, event_schemas, feedback, quiz_schemas, site_visit, user_action 등.
- FastAPI/SQLAlchemy 관련 경고 제거: 기존 변경분 유지(Declarative Base import 경로, lifespan 사용 등) + 스키마 구성 정리.
- Frontend TypeScript type-check 시도: npm ci 후 tsc 실행 결과, React 19 타입 사용 방식과 관련된 광범위 오류 다수 확인(React.ComponentProps, forwardRef 제네릭 등). 현 상태에선 대규모 타입 정비가 필요. 단기 방안과 권장 조치:
  - 단기: 프론트 빌드/개발은 가능하나, 엄격 타입체크는 비활성(임시로 CI에서 type-check 제외) 또는 @types/react/next 버전 고정 전략 검토.
  - 권장: React 19 권장 패턴으로 타입 사용 정리(import type { ComponentProps, ElementRef } from 'react'), React.ComponentProps 네임스페이스 의존 제거, forwardRef 제네릭 표기 정비. 프론트 전역 일괄 치환 작업 필요.
- 다음 액션: (1) 백엔드 테스트 재실행(현재 커스텀 스위트 기준), (2) 프론트 타입 정비 분할 적용 및 tsconfig/의존성 정렬, (3) 문서 최종 반영.
...
- [] **롤백/다운그레이드 정상 동작** (운영/테스트 환경 모두)
  - [] PostgreSQL 환경에서 롤백 테스트 (운영 DB 실전 필요, 기본 롤백 정상)
...
- [] **Kafka/Zookeeper 컨테이너 정상 기동** (docker-compose, 네트워크, 포트, 네임 충돌 해결)
   - [] **FastAPI Kafka 연동** (kafka-python/aiokafka 등 라이브러리 설치, 연결 설정, 샘플 Producer/Consumer 구현)
 - [] **Kafka 연동 자동화 테스트/문서화** (pytest, integration test, 운영/테스트 환경 분리)
 
 ## Kafka 연동 단계 진행상황
 - [] **Kafka 연결 설정**: Docker Compose 설정 완료, Bitnami Kafka 3.6.0 + Zookeeper 구성
 - [] **토픽 정의 및 생성**: 6개 토픽 생성 (user_actions, game_events, notifications, analytics, real_time_feedback, test_kafka_integration)
 - [] **Producer 구현**: CasinoClustersKafkaProducer 클래스 구현, JSON 직렬화, 압축, 재시도 로직
 - [] **Consumer 그룹 설정**: 동일/다른 그룹 간 메시지 분산/복제 테스트 통과
 - [] **오류 처리 및 재시도 메커니즘**: 타임아웃, 잘못된 토픽, 네트워크 에러 처리 구현
 - [] **통합 테스트 자동화**: 1
 - [] **Casino-Club F2P 특화 구현**: 사용자 액션, 게임 이벤트, 알림, 분석, 실시간 피드백 메시지 타입 정의
 - [] **FastAPI 통합**: API 엔드포인트 구현 (/api/kafka/*)
   -  토픽 관리 API (생성, 조회)
   -  메시지 전송 API (사용자 액션, 게임 이벤트, 알림, 분석, 피드백)
   -  실제 게임 시나리오 API (슬롯 스핀, 가챠 오픈, 사이버 토큰 획득)
   -  헬스체크 및 설정 조회 API
 - [] **문서화**: 테스트 결과 문서, 샘플 코드, 사용 가이드 작성
 - [] **개발(SQLite) vs 운영(PostgreSQL) 환경 분리**: Docker Compose 환경별 설정 완료
