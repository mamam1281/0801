...
except Exception:  # Fallback: minimal env-only settings to avoid crashes
    """
    Compatibility shim for configuration.

    This module re-exports settings from app.core.config and provides a few
    legacy attribute names for backward compatibility while code migrates.
    Target: standardize on `from app.core.config import settings`.
    """
