...
export function ShopScreen({
  ...
  useEffect(() => {
    ...
    load();
    // listen to admin catalog updates
    const off = onEvent('shopCatalogUpdated', async () => {
      try {
        const fresh: any = await api.get('shop/catalog');
        if (Array.isArray(fresh)) setCatalog(fresh);
      } catch {
        // ignore
      }
    });
