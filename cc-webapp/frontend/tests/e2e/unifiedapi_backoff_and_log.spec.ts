...
test.describe('unifiedApi: backoff & log suppression', () => {
  test('log gate via localStorage suppresses init logs', async ({ page }) => {
  ...
  test('retry on 429 (if test endpoint available)', async ({ page }) => {
  ...
  test('retry on 500 (if test endpoint available)', async ({ page }) => {
