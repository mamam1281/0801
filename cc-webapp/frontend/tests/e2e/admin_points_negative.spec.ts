...
test('Admin Points: non-admin cannot grant gold (401/403), no-token → 401', async () => {
  ...
  expect([401]).toContain(rNoAuth.status());
...
test('Admin Points: invalid amount returns 400/422 (admin elevated)', async () => {
  const ctx = await request.newContext();
