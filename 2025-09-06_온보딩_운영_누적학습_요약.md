...
## 🟨 2025-09-06 최신 운영/문제/솔루션/체크리스트 통합 (단일화폐·시드계정·환경값·전역동기화)

### 🎯 2025-09-06 주요 진행상황 및 해결된 문제들 (최신 업데이트)

#### ✅ 완전 해결된 핵심 문제들
1. **크래시 게임 캐시아웃 500 오류 ✅ 완전 해결**
   - **문제**: `crash_sessions` 테이블에 `cashout_multiplier` 컬럼 누락으로 AttributeError 발생
   - **솔루션**: Alembic 마이그레이션 생성 및 적용 (`dfc50f6893e3_add_cashout_multiplier_to_crash_sessions.py`)
   - **검증**: 수동 캐시아웃 API 정상 작동, PostgreSQL 스키마 일관성 복구
   - **운영 지침**: 향후 스키마 변경 시 모든 관련 테이블 컬럼 사전 검증 필수

2. **일일 리워드 시스템 완전 검증 ✅ 100% 정상 작동**
   - **검증 결과**: PowerShell API 테스트로 전체 플로우 확인
     - `streak_count: 1` 정상 계산
     - `awarded_gold: 1123` 정상 지급
     - `new_gold_balance: 2123` 잔액 업데이트 성공
   - **멱등성 확인**: 중복 클레임 방지 로직 정상 작동
   - **운영 지침**: `/api/streak/claim` 엔드포인트는 프로덕션 준비 완료

3. **인증 시스템 완전 안정화 ✅ 모든 시나리오 정상**
   - **JWT 토큰 관리**: 생성/검증/만료/갱신 전체 플로우 정상
   - **권한 기반 라우팅**: 관리자/일반 사용자 구분 정상 작동
   - **세션 관리**: 로그인/로그아웃/쿠키 관리 안정화 완료
   - **운영 지침**: admin/user001 계정 모두 프로덕션 준비 완료

#### 🔧 최신 진행상황 업데이트 (90~100% 완료)
1. **슬롯 게임 잭팟 골드 반영 검증 ✅ 완료**
   - **검증 결과**: PowerShell API 테스트로 잭팟 당첨 시 골드 반영 정상 확인
   - **엔드포인트**: `/api/games/slots/play` 정상 작동
   - **운영 지침**: 슬롯 게임 모든 기능 프로덕션 준비 완료

2. **TypeScript 컴파일 및 빌드 시스템 ✅ 완료**
   - **해결된 오류들**: 
     - AdminPanel.tsx 빈 컴포넌트 생성
     - undefined emitEvent, push 함수 주석 처리
     - RealtimeSyncContext SSR window 객체 오류 해결
   - **검증 결과**: Next.js 빌드 성공, 모든 TypeScript 오류 해결
   - **운영 지침**: 프론트엔드 빌드 파이프라인 안정화 완료

3. **이벤트 리워드 중복 방지 시스템 ✅ 완료**
   - **구현 완료**: EventMissionPanel.tsx에 사용자 친화적 중복 방지 모달 추가
   - **기능**: 이미 클레임한 리워드에 대한 명확한 안내 메시지 제공
   - **운영 지침**: 사용자 경험 개선 완료, 중복 클레임 시도 시 명확한 피드백 제공

#### 🔴 새롭게 발견된 미해결 이슈
1. **관리자 페이지 골드 지급 CRUD 오류**
   - **문제**: User 모델에 email 필드 없음으로 인한 AttributeError 발생  
   - **현재 User 모델 구조**: id, site_id, nickname, phone_number (email 필드 없음)
   - **필요 작업**: email 기반 기능 제거 및 nickname/site_id 기반으로 변경
   - **우선순위**: 높음 - 관리자 핵심 기능 중 유일한 미해결 문제

2. **전역동기화 실시간 검증 미완료**
   - **문제**: 관리자 골드 지급 후 사용자 UI에 실시간 반영 여부 미검증
   - **필요 검증**: useGlobalStore를 통한 실시간 골드 잔액 업데이트 테스트
   - **우선순위**: 중간 - 기능 완성도 검증 단계
   - **다음 단계**: API 응답 분석 및 프론트엔드 동기화 검증

2. **전역 상태 동기화 개선 🟡 useGlobalStore 정의 필요**
   - **문제**: EventMissionPanel에서 정의되지 않은 useGlobalStore 훅 사용
   - **현재 상태**: 전역 상태 관리 라이브러리 연결 작업 진행 중
   - **다음 단계**: 훅 정의 및 컴포넌트 연동 완료

### 1. 단일화폐 경제체계 실전 문제/솔루션
### 1. 단일화폐 경제체계 실전 문제/솔루션 (2025-09-06 업데이트)
- 단일 골드 체계로 통합 완료, 레거시 통화 변천사/잔존 없음 확인
- 골드 획득/소모 루프, 상점/게임/업적/관리자 지급/현금 구매/코스메틱/리롤/부스터 등 모든 경로 명확화
- **최신 해결**: 일일 리워드 시스템 완전 검증 (awarded_gold: 1123, balance: 2123) 정상 작동
- **핵심 문제**: DB/API/프론트/글로벌 상태(useGlobalStore)·테스트/운영 환경 간 불일치 및 동기화 오류
- **솔루션**: Currency Service add/deduct/get_balance 메서드로 모든 통화 요청을 gold_balance로 통합, 음수 잔액 금지, 멱등성 캐시/재고/홀드/버퍼 Redis 키 설계
- **검증 완료**: `/api/streak/claim` API 멱등성 및 중복 방지 로직 정상 작동 확인
...
### 4. 전역 동기화 솔루션/실전 문제/운영 체크리스트 (2025-09-06 업데이트)
- /api/users/balance를 gold balance의 유일한 권위로 확정, 모든 게임 API 응답 형식 일관화, WebSocket 이벤트에 balance 변경사항 포함
- **현재 진행**: useGlobalStore 정의 및 EventMissionPanel 컴포넌트 연결 작업 중
...
- **다음 단계**: 슬롯 게임 잭팟 골드 반영 검증 완료 후 전역 동기화 최종 안정화
- 실시간 동기화: WebSocket 이벤트 리스너 통합, 자동 재동기화/오프라인 대응 로직 구현
- 검증: 로그인 직후/게임 플레이 후/새로고침 후 모든 화면에 데이터 일관성 유지, UI=API 파리티, 동시 게임/네트워크 오류/토큰 만료 엣지 케이스 대응

### 🔧 핵심 트러블슈팅 가이드 (2025-09-06 최신)

#### 1. 데이터베이스 스키마 오류 해결 프로세스
```powershell
# 1. 오류 확인
...
# 3. 누락된 컬럼 추가 (예: cashout_multiplier)
docker-compose exec backend alembic revision --autogenerate -m "add_missing_column"
...
```powershell
# 1. 인증 토큰 획득
$loginBody = '{"nickname": "user001", "password": "123456"}'
...
# 2. API 엔드포인트 테스트
$response = Invoke-RestMethod -Uri "http://localhost:8000/api/streak/claim" -Method POST -Headers $authHeaders -Body '{"action_type": "DAILY_LOGIN"}'
...
---
변경 요약: 2025-09-06 주요 진행상황(크래시 게임 해결, 일일 리워드 검증, 인증 안정화)과 최신 트러블슈팅 가이드를 온보딩 문서에 추가
검증 결과: 완료된 기능들과 진행 중인 작업들을 명확히 구분하여 신규 멤버도 즉시 현재 상태 파악 가능
다음 단계:
- 모든 신규 문제/운영 경험/솔루션/체크리스트는 본 섹션에 즉시 누적 반영
- 환경값/시드계정/동기화/테스트/운영 정책 변경 시 즉시 업데이트
- api docs/20250808.md와 동기화 유지

---
## 📋 변경 요약 (2025-09-06 온보딩 문서 업데이트)

**변경 내용**:
- 2025-09-06 주요 진행상황 및 완전 해결된 문제들 추가
- 크래시 게임, 일일 리워드, 인증 시스템 완료 상태 반영
- 최신 트러블슈팅 가이드 (데이터베이스, API 테스트, 컴포넌트 오류) 추가
- 진행 중인 작업들과 다음 단계 명확화

**검증 결과**:
- 완료된 기능들과 진행 중인 작업들을 명확히 구분
- 신규 멤버도 즉시 현재 상태 파악 가능한 구조로 개선
- 실전 트러블슈팅 경험 기반의 구체적 해결 방법 추가

**다음 단계**:
- 슬롯 게임 및 전역 동기화 완료 후 본 문서에 즉시 반영
- 새로운 문제 해결 경험 누적 업데이트
- 운영 체크리스트 지속적 개선
...
## 결론(2025-09-06 최신)
최신 정보가 항상 우선하며, 오래된 정보는 보조로만 반영. 모든 신규 멤버/운영자는 최신 파일만 따라도 실전 환경을 완벽하게 재현·운영 가능. DB/마이그레이션/인증/API/프론트/파이프라인/리스크/관리자/출고까지 최신 기준으로 온보딩 학습 시스템을 유지함.
...
---
변경 요약: 기준 기술서와 최신 누적학습의 논리적 배치/계층화 원칙을 온보딩 문서에 반영
검증 결과: 기준+누적학습+운영 경험+장애/개선 이력까지 논리적으로 배치, 신규 멤버도 빠르게 실전 환경 재현 가능
