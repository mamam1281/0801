...
function Detect-Compose {
  try { & docker compose version *> $null; return @{ cmd='docker'; sub='compose' } } catch {}
  try { & docker-compose --version *> $null; return @{ cmd='docker-compose'; sub=$null } } catch {}
  throw 'Docker Compose가 감지되지 않았습니다. Docker Desktop을 업데이트하세요.'
}

function Exec-InBackend {
  param(
  ...
  $code = $LASTEXITCODE
  if ($code -ne 0) {
    Write-Host ("✖ 실패 (Exit {0}) : {1}" -f $code, $Title) -ForegroundColor Red
  } else {
  $label = $Title
...
# 1) pytest 스모크/가드
$code = Exec-InBackend -Title 'Pytest 스모크/가드 (health, alembic, schema drift)' -Cmd "pytest -q app/tests/test_smoke_health.py app/tests/test_ci_alembic_guard.py app/tests/test_schema_drift_guard.py"
if ($code -ne 0) { $failures += 'pytest_smoke_and_guards' }

# 2) Alembic heads 및 업그레이드
$code = Exec-InBackend -Title 'Alembic heads 확인' -Cmd 'alembic heads -v'
if ($code -ne 0) { $failures += 'alembic_heads' }

$code = Exec-InBackend -Title 'Alembic upgrade head' -Cmd 'alembic upgrade head'
if ($code -ne 0) { $failures += 'alembic_upgrade' }

# 3) OpenAPI 재수출 + drift diff
$code = Exec-InBackend -Title 'OpenAPI 재수출' -Cmd 'python -m app.export_openapi'
if ($code -ne 0) { $failures += 'export_openapi' }

$code = Exec-InBackend -Title 'OpenAPI 드리프트 가드' -Cmd 'pytest -q app/tests/test_openapi_diff_ci.py'
