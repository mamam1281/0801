    ...
    try {
      ...
      dispatch({
        ...
        payload: {
          ...
          balance: { gold: balance.data.cyber_token_balance },
          gameStats: stats.data,
          lastSyncAt: Date.now()
        }
      });
    } catch (error) {
...
### Phase 2: GlobalStore 통합 (2일)
1. GlobalStore에 모든 상태 통합
2. useGlobalSync 훅 구현
3. 기존 분산된 훅들 제거

### Phase 3: 컴포넌트 마이그레이션 (3일)
...
### 동기화 테스트
- [ ] 로그인 직후 모든 화면에 같은 gold 표시
- [ ] 게임 플레이 후 모든 화면에 즉시 반영
- [ ] 프로필 수정 시 모든 화면에 반영
- [ ] 새로고침 후에도 데이터 일관성 유지
