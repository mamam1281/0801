...
- [x] /api/games/stats/me 포맷 최종 단일화(백엔드) 또는 셀렉터 범위 최종 확정(FE)
 ...
 [x] 분산 훅 스윕 완료 → 대체 경로 연결 후 제거
  ...
  - [ ] 클린 빌드 후 산출물 정리(.next)
    - [ ] 문서 갱신(정리 완료 시 체크)
 [ ] E2E_REQUIRE_STATS_PARITY=1 전환(2회 연속 200 + 3회 연속 UI=API PASS 확인 후)
  - [x] 컨테이너 Playwright 연속 3회 그린 기록(현재: 3/3)
  - [x] frontend/components/TokenBalanceWidget.tsx
 [ ] E2E_REQUIRE_STATS_PARITY=1 승격 → 그린 1회 추가 + deep 1회 선택 실행.
 [ ] api docs/20250808.md에 클린 빌드/산출물 정리 완료 후 체크 반영 및 필요 시 OpenAPI 재수출.
  ...
  - [ ] 1회 빌드/스모크 통과 후 산출물 정리(.next)
    - [ ] 삭제 PR 병합 및 문서 갱신
    
- [ ] E2E_REQUIRE_STATS_PARITY=1 전환(2회 연속 200 + 3회 연속 UI=API PASS 확인 후)
  - 검증 절차: (a) `/api/games/stats/me` 200 연속 2회 (b) UI 합계=testid(`games-stats-total` 등) 기준 API와 일치 3회 연속
  - 하위 작업:
  - [x] `/api/games/stats/me` 200 확인 #1
    ...
    - [ ] UI=API 파리티 PASS #2
    - [ ] UI=API 파리티 PASS #3
  - [ ] 컨테이너 Playwright 연속 3회 그린 기록(현재: 1/3)
    - [ ] env 플래그 승격: STRICT_STATS_PARITY=1
  - 운영 절차: 컨테이너 Playwright 3회 연속 실행 성공 기록 후 env 플래그 승격(STRICT_STATS_PARITY=1)

### 분산 훅 스윕 리스트(대체 경로/제거 체크)
- 유지: useGlobalSync(권위 동기화), useGlobalUser(스토어 셀렉터), useRealtimeSync(WS 반영)
- 대체 연결 후 제거 대상:
  - useBalanceSync → useGlobalSync.syncBalance 사용으로 대체 [ ] 제거 준비 [x] 호출처 대체 [ ] 삭제(빌드 산출물 정리 필요)
  - 진행: 호출처 대체 완료(4/4). .next 재빌드 후 참조 제거 예정.
...
- E2E 컨테이너 테스트 연속 그린 목표: 현재 1/3. 추가 2회 실행 예정.

### 검증(누적)
- pytest: 백엔드 GameStats 관련 테스트 그린 유지(세션 격리, flush-only 정책).
- Playwright: 컨테이너 1회 그린 확인. 추가 2회 예정.
- Alembic heads: 단일 head 유지(수동 확인 필요 시 docker 내에서 재검증).

### 다음 단계(단기)
- [ ] Playwright 컨테이너 2회 추가 실행하여 3연속 그린 확보.
- [ ] E2E_REQUIRE_STATS_PARITY=1 승격 검토 → 그린 1회 추가 + deep 1회 선택 실행.
