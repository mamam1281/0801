...
- [ ] 분산 훅 스윕 완료 → 대체 경로 연결 후 제거
  - 실행 기준: 각 훅 호출처를 `useGlobalSync`/스토어 셀렉터로 대체 후 1회 빌드/스모크 통과 시 제거 PR 분리 제출
  - 1차 타깃: useBalanceSync → syncBalance, useDashboard → useGameStats 조합(소스 주석에 @deprecated 표기)
  - 하위 작업:
    - [x] 호출처 스캔 및 목록화 (2025-09-01)
      - useBalanceSync 호출처
  - [x] frontend/components/TokenBalanceWidget.tsx
  - [x] frontend/components/ShopScreen.tsx
  ...
  - [x] useDashboard에 @deprecated 주석 추가
    - [ ] 호출처 대체(useGlobalSync.syncBalance / useGameStats)
    - [ ] 1회 빌드/스모크 통과
    - [ ] 삭제 PR 제출
...
- 대체 연결 후 제거 대상:
  - useBalanceSync → useGlobalSync.syncBalance 사용으로 대체 [ ] 제거 준비 [ ] 호출처 대체 [ ] 삭제
  - 진행: 호출처 대체 완료(4/4). 후속 PR에서 훅 파일 제거 예정.
  - useDashboard → useGameStats/useGlobalStore 조합으로 대체 [ ] 대체 [ ] 삭제
