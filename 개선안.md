...
- 권위 소스(Authoritative Sources)
  ...
  - 보상/이벤트: POST /api/rewards/distribute, /api/events/... (claim/progress)
- 실시간 전파(브로드캐스트)
...
### 공통 레이아웃(App.tsx)
- [ ] 최초 진입 시: GET /users/profile, GET /users/balance, GET /games/stats/me 동시 호출 → store.hydrateFromServer()
- [ ] WS 연결: useRealtimeSync 활성화, 이벤트별 액션 매핑 점검
- [ ] 사이드메뉴/하단탭: store 셀렉터만 사용(임의 user state 금지)

### 메인(HomeDashboard)
- [ ] 최근 액션/커뮤니티/메트릭: 읽기 전용. 개인 데이터 표시는 전부 store 셀렉터 사용
- [ ] 골드/레벨/스트릭: profile_update 수신 시 즉시 반영
...
### 상점(ShopScreen)
- [ ] 카탈로그는 서버 응답 기반. 가격/골드 하드코딩 금지
- [ ] 구매 성공 시: 서버 응답의 new_gold_balance 또는 profile_update 이벤트로 store 갱신
...
- 게임 전환(로컬 가산 제거 & withReconcile)
  - [ ] NeonSlotGame: 스핀 성공 시 서버 응답 반영 → `withReconcile`; 실패 시 재동기화
  - [ ] RockPaperScissorsGame: 플레이 성공 반영 → `withReconcile`
  - [ ] GachaSystem: 스핀/보상 수령 반영 → inventory는 store 액션으로
  - [ ] NeonCrashGame: bet/start/cashout 단계별 서버 권위 반영 → `withReconcile`

- 상점/프로필 전환
  - [ ] ShopScreen: 카탈로그 서버 기반; 구매는 `withReconcile`; 이벤트 반영
  - [ ] PurchaseModal: 멱등키 헤더 적용, 영수증/오류 표시, 재시도 처리
  - [ ] InventoryScreen: store.inventory 구독, 불일치 시 재조회
  - [ ] ProfileScreen: selector 사용, 편집 후 서버 응답으로 덮어쓰기
