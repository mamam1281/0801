...
## 2025-08-27 E2E 실패 원인 및 수정

- 증상
  - auth_migration.spec.ts: 레거시 토큰 → 번들 마이그레이션 후 cc_auth_tokens 부재로 실패.
  - ws_profile_update_smoke.spec.ts: /api/auth/me 401 → Authorization 누락.

- 원인
  - 초기 렌더 타이밍에 따라 번들 생성 전 /api 호출이 먼저 수행될 수 있음.
  - fetch로 수행되는 상대경로(/api/*) 호출에 Authorization 자동 주입이 보장되지 않음.

- 조치
  1) `app/App.tsx` 최상단에서 `ensureTokenBundleMigrated()`를 즉시 실행하여 번들 보장.
  2) 전역 fetch 패치: `/api/*` 요청에 Authorization 헤더가 없으면 cc_auth_tokens(또는 cc_at 쿠키)에서 Bearer 주입.
  3) 기존 useEffect의 보조 호출은 유지(보수적 중복 호출).

- 기대 효과
  - 레거시 토큰만 존재해도 홈 진입 즉시 번들 생성되어 테스트의 토큰 길이 검증 통과.
  - Playwright `page.request.get('/api/auth/me')` 경로에서도 Authorization이 자동 부착되어 200 기대치 충족.

- 추후 과제
  - SSR 경로에서는 서버 fetch 라우팅이므로 서버 측 미들웨어 주입(또는 Next.js middleware)로 보강 고려.
  - 토큰 쿠키(cc_at) 공식화 여부 검토 및 보안 속성 설정(HttpOnly, Secure 등) 계획 수립.
...
### 프로필(ProfileScreen)
- [x] GET /auth/me, /users/balance, /games/stats/me 3종을 store에서 구독 (selectors/useRealtime + fetchProfileBundle→mergeProfile/mergeGameStats 적용)
- [x] 수정(닉네임 등) 성공 시
- [x] +-------: 서버 새 프로필로 store 덮어쓰기 (withReconcile + mergeProfile(dispatch, serverPayload))
  - 구현: `frontend/hooks/useProfileUpdate.ts`의 `updateProfile/updateNickname`이 `api.put('users/profile', ...)` 호출 후 응답 payload를 `mergeProfile`로 반영하고, `useWithReconcile`로 최종 하이드레이트 수행.
  - 사용처: `components/SettingsScreen.tsx` 닉네임 저장 버튼 → `useProfileUpdate().updateNickname()` 호출 경로.
...
### 2025-08-26 추가 수정: 전역 스토어 중복 파일 제거 및 SSR 500 해소

- 변경 요약
  - `frontend/store/globalStore.tsx` 중복 파일 제거 → 단일 소스 `frontend/store/globalStore.ts`만 유지(mergeProfile/mergeGameStats/applyPurchase 액션 정상 export).
  - SSR 500("useGlobalStore must be used within GlobalStoreProvider")는 Provider 트리 순서/위치 문제였으며, `app/providers.tsx`는 UI 전용 Provider만 유지하고, `GlobalStoreProvider/EnsureHydrated/RealtimeSyncProvider`는 `app/App.tsx` 내부에만 배치하도록 고정.

- 검증
  - 프론트엔드 재빌드 후 import 에러(mergeProfile/mergeGameStats 등) 해소 예상. 컨테이너 기동 시 GET / 200 확인 예정.
  - Playwright 러너의 페이지 루트 접근 시 SSR 훅 호출로 인한 예외가 더 이상 발생하지 않아야 함.

- 다음 단계
  - `docker-manage.ps1 start` 후 `docker-manage.ps1 logs frontend`로 GET / 200 확인.
  - 컨테이너 내부에서 Playwright 스위트 재실행해 WS→UI 스모크 및 토큰 마이그레이션 케이스 그린화 확인.
...
추가 변경(2025-08-26):
- 백엔드 FastAPI에 `/api/health` 별칭 엔드포인트 추가(기존 `/health`와 동일). 일부 스크립트/러너가 `/api/health`를 조회하던 히스토리를 흡수하여 컨테이너 E2E 글로벌 셋업 초기 5xx 이슈를 차단.
- 검증: 컨테이너 기동 후 `GET /health` 200, `GET /api/health` 200 확인. docker-compose 헬스체크는 `/health` 지속 사용, Playwright 글로벌 셋업은 `/health` 우선 + 루트 폴백 유지.
- 다음 단계: docker-compose(-f docker-compose.playwright.yml)로 전체 스위트 재실행해 그린 확인. 문서 전반에서 권장 경로는 `/health`로 통일, `/api/health`는 호환 목적 유지.