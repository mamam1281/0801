...
- 2025-08-26: 프론트 컨테이너 런타임 ENOENT(.next/routes-manifest.json) 이슈 해결. Dockerfile(프로덕션)에서 standalone 출력 외 필수 매니페스트/BUILD_ID 동봉, dev start 스크립트(.docker/start-dev.sh)에서 부분 캐시 감지시 .next 정리 추가. docker compose 재빌드/기동 후 GET / 200 확인.
 - 2025-08-26: Backend: 가챠 분포 테스트 관련 토큰 부족 진단 및 테스트 보정(테스트 토큰 충전 후 단일 분포 테스트 통과). 기록: 단기 트러블슈팅 항목으로 개선안 문서에 추가.
...
### 프로필(ProfileScreen)
- [x] GET /auth/me, /users/balance, /games/stats/me 3종을 store에서 store.hydrateFromServer()로 구독/초기화
 - [x] 프로필 수정(닉네임 등) 성공 시 서버에서 반환한 최신 프로필로 store 덮어쓰기
 - [x] 프로필 편집 후의 UI/로컬 상태는 반드시 store 액션으로만 갱신(로컬 직접 수정 금지)

검증: `ProfileScreen.tsx`에 `submitProfileEdit` 함수를 추가하여 `unifiedApi.put('auth/me', patch)` 응답을 `setProfile(dispatch, updatedProfile)`로 덮어쓰도록 구현함. 로컬 임시 편집은 `localUser`로 유지하되, 최종 확정은 전역 store로 반영되도록 조정.
...
검증 요약 (이벤트/보상 변경)
- 변경 요약: `EventMissionPanel`과 `RewardContainer`의 보상 수령/클레임 경로에서 서버 응답의 `awarded_gold`/`awarded_gems`를 감지하면 전역 스토어의 `applyReward(dispatch, {...})`를 즉시 호출하도록 구현하였고, WS `reward_granted` 수신 시 동일 로직으로 처리되도록 `lib/sync.ts`에 동적 호출을 추가했습니다.
- 검증 결과: 단위 테스트(히스토리 스모크)는 컨테이너 환경에서 PASS를 확인했고, 로컬 머신의 직접 Jest 실행은 `next/jest` 의존성 문제로 실패(환경 의존)하여 컨테이너 내 검증이 권장됩니다. 컨테이너에서의 프로덕션 빌드는 컴파일 성공(경고 존재)으로 확인되었습니다.
- 다음 검증 작업: (1) 컨테이너 내부에서 전체 프론트엔드 테스트 및 추가 단위 테스트(보상 claim → `applyReward` 동작 확인) 실행, (2) WS reward_granted E2E 스모크 추가 및 Playwright 스펙 확장.

### 어드민(AdminPanel/ShopManager)
- [x] 골드 지급/차감 액션 성공 시: 해당 유저 세션에 profile_update 브로드캐스트 전제 → 현재 세션이면 store.reconcileBalance() (구현 완료)
- [x] 카탈로그/패키지 업서트는 프론트 캐시 무효화 및 새 목록 재조회 (구현 완료)

실행 지침(권장)
- 골드 조정(Admin) -> profile_update 브로드캐스트: 서버는 `profile_update` 이벤트의 payload에 `target_user_id` 또는 `user_id` 필드를 포함하여 브로드캐스트해야 합니다. 프론트는 `RealtimeSyncProvider`에서 해당 필드가 현재 전역 프로필(`state.profile.id`)과 일치하면 `reconcileBalance(dispatch)`를 호출하여 즉시 서버 권위로 재동기화하도록 구현되어야 합니다(이미 `globalStore.reconcileBalance` 헬퍼가 제공됨).
- 카탈로그/패키지 업서트: 클라이언트 캐시(React Query/로컬 메모리/컴포넌트 상태)를 사용 중이면 업서트 성공 응답 후 다음 단계를 수행하세요:
  1) 업서트 성공 응답을 받은 직후 프론트에 캐시 무효화 신호(예: `invalidateCache('shopCatalog')`)를 호출합니다.
  2) 무효화 후 새 목록 재조회(`unifiedApi.get('shop/catalog')`)를 수행하여 최신 카탈로그를 전역 상태 또는 로컬 컴포넌트로 반영합니다.
  3) 가능하면 업서트 API는 변경된 리소스의 `etag` 또는 `version`을 반환하여 클라이언트가 선택적 부분 업데이트를 수행할 수 있게 합니다.

테스트/검증
- 어드민 골드 조정 E2E: Admin에서 타 유저의 골드를 조정할 때(컨트롤러 시나리오), 대상 유저의 클라이언트가 열려 있으면 실시간으로 `profile_update` 수신 후 `reconcileBalance` 호출로 잔액이 최신화되는지 Playwright로 검증하세요.
- 카탈로그 업서트 E2E: 업서트 직후 관리 화면과 유저 화면에서 카탈로그 변동이 반영되는지, 캐시 무효화 후 재조회가 정상 동작하는지 검증하세요.
...
변경 요약/검증/다음 단계:
- 변경 요약: `app/providers.tsx`에서 providers 순서를 `GlobalStoreProvider` → `RealtimeSyncProvider` → `EnsureHydrated` → `ToastProvider`로 고정했고, `app/App.tsx`의 중복 provider 래핑을 제거하여 최상위 `Providers`를 단일 출처로 통일했습니다. `TokenBalanceWidget`과 `NotificationClient`를 확인해 폴링(setInterval) 사용이 없음을 확인했습니다.
- 검증 결과: 컨테이너 내부에서 `npx jest components/__tests__/ProfileHistory.test.tsx --env=jsdom` 실행 결과 테스트 통과(PASS). `TokenBalanceWidget`은 마운트 시 옵션성 1회 `reconcileBalance()` 호출만 수행하며 주기적 폴링은 사용하지 않습니다. `NotificationClient`는 WebSocket을 사용하고 폴링이 없습니다.
- 다음 단계: (1) 헤더/토큰 위젯이 selector만 사용하도록 추가 스모크(Playwright) 작성 및 컨테이너 실행, (2) 앱 런타임 스모크(`/health`, `/docs`) 실행 및 OpenAPI 재내보내기 필요 시 수행, (3) PR 생성 및 `api docs/20250808.md`에 변경 요약 병기.
- TokenBalanceWidget
...
변경 요약 (2025-08-26): RealtimeSyncContext에서 클라이언트 전용 훅(useToast) 직접 호출을 제거하고 대신 전역 CustomEvent('app:notification')를 발행하도록 수정하였습니다(기존 NotificationToast가 수신 및 중복 억제 담당). TokenBalanceWidget는 전역 store의 canonical balance를 우선 표시하며 클릭 시 최근 변경 이력 모달을 띄울 수 있도록 보강했습니다. unifiedApi는 '/api' 접두 자동 처리, 401 리프레시 및 재시도/멱등키 정책을 충족하는 것으로 검토되어 코드 변경은 없습니다.

> 검증(로컬 스모크): `ToastProvider`의 1.5초 중복 억제와 `app:notification` 이벤트 연계를 로컬 스모크로 확인했습니다 (show → suppress within 1.5s → show after delay). Playwright E2E는 CI/docker-compose.playwright 네트워크에서 실행해 주세요(로컬 네트워크/네임스페이스 차이로 인한 접근성 문제 발생 가능).
...
### 최근 변경 요약
변경 요약 (2025-08-26):
- ProfileHistory 컴포넌트 추가: /cc-webapp/frontend/components/ProfileHistory.tsx — cursor 기반 페이지네이션(merge/replace 모드), `unifiedApi` 사용으로 API 호출 분리.
- 프런트엔드 프로덕션 빌드(컨테이너 내부) 실행: 빌드 성공(컴파일 완료). 다만 프로젝트 전역에서 `@typescript-eslint/no-explicit-any` 및 일부 unused/dep 경고 다수 보고됨.
- Jest 스모크 테스트 추가 및 실행: `components/__tests__/ProfileHistory.test.tsx` — 테스트 통과(PASS).

검증 결과:
- Next production build: Compiled successfully; `/shop/history` 라우트 생성 및 정적 생성 완료(경고는 존재).
- Jest: ProfileHistory 스모크 테스트 1개 통과.

다음 단계:
1. PR 준비 및 커밋: 변경 파일(`components/ProfileHistory.tsx`, `components/__tests__/ProfileHistory.test.tsx`, `개선안.md`) 커밋 및 PR 생성(요약/검증/다음 단계 포함).
2. Playwright E2E 스모크 테스트 추가(선택): `/shop/history` 방문 후 핵심 요소 확인 테스트 작성 및 컨테이너 내 실행.
3. 중장기 과제: 프로젝트 전역의 `any` 경고와 React 훅 의존성 경고 정리(범위가 큼 — 우선순위 목록 제안 필요).
