...
- layout.tsx / Providers
  - [x] StoreProvider, RealtimeSyncProvider, ToastProvider 순서 보장(App.tsx에서 Store→EnsureHydrated→Realtime→Toast 구성, layout/providers는 UI 전용)
...
- lib/sync.ts
  - [x] withReconcile(action): 성공/실패 무관 balance 재조정
  - [x] withIdem(action,key): 멱등키 부여 → unifiedApi(X-Idempotency-Key 자동 주입)로 대체 구현
...
### 2025-08-27 (E2E 안정화/보상 즉시 반영/토큰 경로 하드닝)

- 변경 요약
  - 이벤트/보상: `EventMissionPanel`에서 claim/claimEvent 성공 시 응답의 `rewards`를 즉시 전역 스토어에 반영(`applyRewardToStore(dispatch, { reward_data: response.rewards })`), 이후 `GET /users/balance`로 서버 권위 재동기화. 진행도는 항상 서버 재조회로만 갱신(클라 가산 금지).
  - 인증 경로 하드닝: `frontend/middleware.ts`에서 `/api/*` 요청 시 쿠키(`cc_auth_tokens`→access_token 또는 `cc_access_token`)를 읽어 `Authorization: Bearer` 자동 주입. `tokenStorage.js`가 번들 저장 시 `cc_access_token` 쿠키를 동기화하고, `layout.tsx`의 early 스크립트가 레거시→번들 마이그레이션 및 쿠키 동기화를 하이드레이션 이전에 수행.
  - E2E 보강: 
    - `auth_migration.spec.ts` 대기(2s) 확대 + 토큰 존재성 검사로 완화 + Authorization 폴백 호출 추가.
    - `ws_profile_update_smoke.spec.ts`에서 테스트 시작 전에 회원가입→토큰 시딩(addInitScript로 localStorage+cookie), `page.request` 비인증 환경에 대비한 Authorization 헤더 폴백, 렌더/검증 대기 소폭 증가, 요청/응답 로깅 추가.

- 검증 결과
  - Playwright(컨테이너) 현재 결과: 21 passed / 1 failed / 2 skipped.
    - 통과: `auth_migration.spec.ts` 포함 대부분 시나리오.
    - 실패: `ws_profile_update_smoke.spec.ts` – 일부 러너에서 `page.request('/api/auth/me')` 가 미들웨어를 경유하지 않아 401 발생. 폴백(Authorization 헤더) 추가 이후에도 간헐적 실패가 남아 추가 보강 예정.
  - 백엔드 헬스/Alembic/OpenAPI: 영향 없음(스키마 변경 無). /health 200, /docs 정상.

- 다음 단계
  - WS 스모크 스펙을 브라우저 내 `fetch('/api/...')` 기반으로 전환하여 미들웨어 경유를 확정(또는 `page.request` 경로엔 항상 Authorization 헤더 부착). 실패 지점만 대기 200~300ms 추가 및 요청/응답/헤더 캡처를 아티팩트로 저장.
  - 프론트 Providers 순서 가이드 정렬: `StoreProvider → EnsureHydrated → RealtimeSyncProvider → ToastProvider`로 재배치(현재 Providers.tsx는 `Toast → RealtimeSync → EnsureHydrated → Feedback` 순서). 실제 StoreProvider는 `App.tsx`에서 감싸고 있어 기능상 문제는 없으나, 가이드와 문서 일치 필요.
  - `api docs/20250808.md`에 이번 변경 요약/검증/다음 단계 추가 기록 및 OpenAPI 재수출 여부 점검(필요 시 갱신).
