...
추가 
### 08-21 작업할것 ###
Read shop.py, lines 150 to 500

Read shop.py, lines 1225 to 1265

Read payment_gateway.py, lines 1 to 160

Read shop.py, lines 1245 to 1298

Read todo list

문제 핵심: 테스트는 /api/shop/buy-limited 경로(신규 확장 로직) 사용, 여기서는 PaymentGateway 결제 흐름 필수 → amount= (pkg.price_cents - 할인) * quantity. 현재 always_success 모드라면 authorize가 True 이어야 하므로 첫 구매 실패 원인은 (a) promo 사용 가능 판정 실패(can_use_promo False) 또는 (b) WINDOW_CLOSED/USER_LIMIT/OUT_OF_STOCK 분기. 실패 body를 아직 출력 안 했습니다(테스트는 success만 검사). 다음 조치 제안:

다음 단계:

실패 응답 JSON 직접 수집 (buy-limited 호출 직후 body 로그) – 이미 shop.py 분기상 reason_code 포함됨.

can_use_promo가 False가 되는 조건: _promo_max_uses에 code=ONEUSE, max_uses=1 등록 후 _promo_used_count가 1 이상이면 False. 초기값 0. 우리가 reset_promo_usage 추가했지만 admin_promo_upsert 직후 reset으로 0 설정. 그런데 test는 buy-limited 라우터 (신규)에서 promo 사용 카운트를 record_promo_use 호출하는 지점은 성공 처리 마지막 부분(1378 근처) → 첫 호출이 success=False이면 promo_used_count 증가 안 함. 따라서 현재 실패 분기는 PROMO_EXHAUSTED 아님(그 경우 success False reason_code PROMO_EXHAUSTED). 원인 좁히기 위해 body 필요.

WINDOW_CLOSED 가능성: admin upsert에서 end_at = now + 30일로 설정, start_at=now. tz정보: admin upsert에서 timezone.utc now 사용, buy-limited 라우터는 pkg.start_at tzinfo 있으면 now=datetime.now(pkg.start_at.tzinfo). 조건 OK.

OUT_OF_STOCK: stock_total=2, try_reserve 실패해야 함. try_reserve는 최초 호출 시 Redis 키 초기화 후 decremented → 성공해야 함.

USER_LIMIT: per_user_limit=2, already=0 → 통과.

남는 것은 Payment 실패 가능성: authorize(total_price_cents, currency) 호출. amount는 할인 적용 후 50. PaymentGateway.authorize always_success에서 success True 반환. 환경 변수 PAYMENT_GATEWAY_MODE 혹시 compose나 테스트에서 always_fail 설정? 확인 필요.

계획:

docker compose exec backend env | grep PAYMENT_GATEWAY_MODE
직접 buy-limited 요청 후 body 출력.
원하시면 제가 바로 실패 응답 수집과 환경변수 확인 명령 실행하겠습니다. 실행 진행할까요? (yes라고 답하면 진행)

추가로 PowerShell에서 Python REPL 내에서 docker compose exec 명령을 치면 SyntaxError가 나는 것은 REPL 안에서 쉘 명령이 아니라 Python 코드로 해석되기 때문입니다. 외부 셸에서 실행하거나 REPL 안에서는 httpx.Client 방법만 사용해야 합니다.

원클릭 재현 파이프라인(셸에서 실행): PowerShell 예: docker compose exec backend python - <<'PY' import httpx, json from main import app c=httpx.Client(transport=httpx.WSGITransport(app=app), base_url='http://test') def signup(site): c.post('/api/auth/signup', json={'site_id':site,'nickname':site,'password':'x','invite_code':'5858','phone_number':'010'+site[-6:].zfill(9)}) signup('admin_dbg3') admin = c.post('/api/auth/login', json={'site_id':'admin_dbg3','password':'x'}).json()['access_token'] c.post('/api/admin/users/elevate', json={'site_id':'admin_dbg3'}) h={'Authorization':f'Bearer {admin}'} c.post('/api/admin/limited-packages/upsert', json={'package_id':'PKG-PROMO-1','name':'Promo Pack','price':100,'stock_total':2,'stock_remaining':2,'per_user_limit':2,'is_active':True,'contents':{'bonus_tokens':10}}, headers=h) c.post('/api/admin/promo-codes/upsert', json={'code':'ONEUSE','package_id':'PKG-PROMO-1','discount_type':'flat','value':50,'max_uses':1,'is_active':True}, headers=h) signup('user_dbg3') u_tok = c.post('/api/auth/login', json={'site_id':'user_dbg3','password':'x'}).json()['access_token'] uh={'Authorization':f'Bearer {u_tok}'} resp = c.post('/api/shop/buy-limited', json={'package_id':'PKG-PROMO-1','promo_code':'ONEUSE'}, headers=uh) print('BODY', resp.text) PY
